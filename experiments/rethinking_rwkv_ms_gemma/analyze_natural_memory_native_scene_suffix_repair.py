#!/usr/bin/env python3
"""Analyze and sign the adaptive local-suffix repair candidate."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_scene_suffix_repair as training,
)
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_scene_suffix_repair_eval as runner,
)


SCHEMA = "rwkv_ms_natural_memory_native_scene_suffix_repair_result.v1"
...
def analyze(**kwargs: Any) -> Mapping[str, Any]:
    ...
    result["study_scope"] = "adaptive_development_only"
    result["external_replication_authorized"] = False
    if result["status"] == "passed":
        result["decision"] = (
            "Adaptive TRAIN-only suffix repair cleared the development gates, "
            "but benchmark-informed method design forbids protected-split authorization."
        )
    else:
        result["decision"] = (
            "Adaptive TRAIN-only suffix repair failed at least one development gate; "
            "archive without protected-split evaluation."
        )
    result["receipt"] = {
        "algorithm": "sha256",
