#!/usr/bin/env python3
"""Run the short donor-focused recurrent-routing continuation stage."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_recurrent_routed_posttrain_stage2 as runner,
)


PROTOCOL = SCRIPT_DIR / "natural_memory_native_recurrent_routed_posttrain_stage3_protocol_v1.json"
PROTOCOL_PAYLOAD_SHA256 = (
    "df4a7fdf814f29d69f65e8f7437766623e2cf526107a552e78aa33f21d2bb4ca"
)
PREDECESSOR_ROOT = SCRIPT_DIR / "local_artifacts/recurrent_routed_posttrain_stage2_train64_v1"
PREDECESSOR_RESULT_RECEIPT = (
    "01ada5458eca9c1f53987862585bba71c3fc7c2832dd737bf77cb095f479e712"
)
PREDECESSOR_ADAPTER_WEIGHTS_SHA256 = (
    "7af3769fa34631329a54fb8caf44797a3a5598344e104680b6aa2cb108339248"
)
PREDECESSOR_ADAPTER_CONFIG_SHA256 = (
    "dc20cbe794479c9f802b45bedef83ff65543445dd85bfdb36419396cadd95d37"
)
DEVELOPMENT_ROOT = SCRIPT_DIR / "local_artifacts/recurrent_routed_posttrain_stage2_development_v1"
DEVELOPMENT_RESULT_RECEIPT = (
    "88129262892d28795b23752d44289133c2f5416245847d944af17c9b4853a47a"
)
TRAIN_UPDATES = 10
PREFLIGHT_UPDATES = 1
LEARNING_RATE = 1e-4
...
TASK_COUNTS_PER_STEP = {"attribution": 2, "narrative": 3, "scene": 3}
...
def validate_lineage() -> tuple[Mapping[str, Any], Mapping[str, Any]]:
    ...
    if (
        ...
        raise ValueError("Stage-3 recurrent-routed lineage differs")
...
def choose_donor(
    ...
    if not candidates:
        raise ValueError(f"Stage-3 target has no donor: {target}")
    return min(
        candidates,
        key=lambda row: (
...
def stage3_schedule(
    ...
    for step in range(1, TRAIN_UPDATES + 1):
        ...
        for task, count in TASK_COUNTS_PER_STEP.items():
            ...
            if len(selected) != count:
                raise RuntimeError(f"Stage-3 has insufficient untouched {task} rows")
            step_rows.extend(selected)
            cursors[task] += count
        step_rows.sort(
            key=lambda row: hashlib.sha256(
                f"rwkv-ms-recurrent-routed-stage3:{step}:{row.row_sha256}".encode(
                    "utf-8"
                )
            ).hexdigest()
        )
        ...
        for position, target in enumerate(step_rows):
            donor = choose_donor(target, rows_by_task[target.task])
    ...
    if (
        ...
        or {task: cursors[task] for task in common.TASKS}
        != {"attribution": 20, "narrative": 30, "scene": 30}
        or any(
            sum(row.prompt_variant == variant for row in schedule) != 20
            for variant in range(4)
        )
    ):
        raise RuntimeError("Stage-3 untouched prompt-balanced schedule differs")
    lineage_hash = common.canonical_sha256(
        {"prior_96_steps": prior_payload, "stage3_steps": payload}
    )
    return tuple(schedule), payload, lineage_hash


def configure_runner() -> None:
    runner.SCHEMA = "rwkv_ms_natural_memory_native_recurrent_routed_posttrain_stage3.v1"
    runner.INPUT_SCHEMA = "rwkv_ms_natural_memory_native_recurrent_routed_posttrain_stage3_input.v1"
    runner.PREFLIGHT_STATUS = "stage3_preflight_passed"
    runner.TRAINING_STATUS = "stage3_training_complete_development_evaluation_authorized"
    runner.FAILURE_STATUS = "stage3_training_failed_development_evaluation_blocked"
    ...
    runner.stage2_schedule = stage3_schedule


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--updates", type=int, required=True, choices=(1, 10))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if context is None:
        raise ValueError("Stage-3 post-training requires four-rank torchrun")
    try:
        result = runner.run(
            context=context,
