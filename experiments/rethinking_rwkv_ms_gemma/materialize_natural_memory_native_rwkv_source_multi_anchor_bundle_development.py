#!/usr/bin/env python3
"""Materialize the fresh open split for multi-anchor RWKV bundle training."""
...
MANIFEST_SCHEMA = (
    "rwkv_ms_natural_memory_native_rwkv_source_multi_anchor_bundle_development.v1"
)
ROW_SCHEMA = (
    "rwkv_ms_natural_memory_native_rwkv_source_multi_anchor_bundle_development_row.v1"
)
SEALED_MANIFEST_SHA256 = (
    "dae77094047eed13492e258698569e119136d49d6796ad941ffb25052994fd5f"
)
...
DEFAULT_PRIOR_DEVELOPMENT_MANIFEST = (
    PROJECT_ROOT
    / "experiments/rethinking_rwkv_ms_gemma/local_artifacts/"
...
def materialize_prepared(
    *,
    ...
    if output_root.exists():
        raise ValueError(f"Multi-anchor development output must be fresh: {output_root}")
    if list(metadata_rows) != parent_materializer._metadata_rows(source_rows):
        raise ValueError("Multi-anchor metadata differs from source rows")
    ...
    if {path.name for path in output_root.iterdir()} != set(FILE_NAMES):
        raise RuntimeError("Multi-anchor development file inventory differs")
    return manifest
...
def materialize(
    *,
    ...
    if source_path.resolve() != expected_source:
        raise ValueError("Multi-anchor development requires the pinned FIT source")
...
def load_manifest(manifest_path: Path) -> dict[str, Any]:
    if manifest_path.name != "manifest.json":
        raise ValueError("Multi-anchor loader accepts only manifest.json")
    payload = manifest_path.read_bytes()
    if not SEALED_MANIFEST_SHA256 or sha256_bytes(payload) != SEALED_MANIFEST_SHA256:
        raise ValueError("Multi-anchor manifest file hash differs")
    manifest = json.loads(payload.decode("utf-8"))
    if not isinstance(manifest, Mapping):
        raise ValueError("Multi-anchor manifest root is not an object")
    if (
        ...
        raise ValueError("Multi-anchor manifest contract differs")
    _validate_receipt(
        manifest,
        payload_scope="canonical_manifest_without_receipt",
        description="Multi-anchor manifest",
    )
    ...
    if (
        ...
        raise ValueError("Multi-anchor manifest inventory differs")
...
def read_open_development(
    ...
    if dict(manifest) != on_disk_manifest:
        raise ValueError("Multi-anchor manifest differs from sealed bytes")
    ...
    if sha256_bytes(payload) != binding["sha256"]:
        raise ValueError("Multi-anchor development file hash differs")
    ...
    for line in lines:
        ...
        if not isinstance(row, Mapping):
            raise ValueError("Multi-anchor row is not an object")
        _validate_receipt(
            row,
            payload_scope="canonical_bundle_row_without_receipt",
            description="Multi-anchor row",
        )
        ...
        if (
            ...
            raise ValueError("Multi-anchor row binding differs")
    ...
    if (
        ...
        raise ValueError("Multi-anchor development payload differs")
    return rows
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--protected-manifest-path", type=Path, default=DEFAULT_PROTECTED_MANIFEST
    )
    parser.add_argument(
        "--prior-development-manifest-path",
        type=Path,
        default=DEFAULT_PRIOR_DEVELOPMENT_MANIFEST,
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    manifest = materialize(
        ...
        prior_development_manifest_path=(
            args.prior_development_manifest_path.expanduser().resolve(strict=True)
        ),
