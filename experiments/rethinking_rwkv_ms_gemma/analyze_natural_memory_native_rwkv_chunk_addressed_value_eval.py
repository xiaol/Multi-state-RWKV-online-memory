#!/usr/bin/env python3
"""Aggregate and sign the chunk-addressed native causal evaluation."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_rwkv_chunk_addressed_value_eval as evaluation,
)


SCHEMA = "rwkv_ms_natural_memory_native_chunk_addressed_value_result.v1"
...
def read_records(
    ...
    if len(partition_dirs) != evaluation.WORLD_SIZE * PARTITIONS_PER_SHARD:
        raise ValueError(
            f"Expected four chunk-addressed partitions, found {len(partition_dirs)}"
        )
    for partition_dir in partition_dirs:
        binding_path = partition_dir / "input_binding.json"
        if not binding_path.is_file():
            raise ValueError(f"Missing chunk-addressed binding: {binding_path}")
        ...
        if (
            ...
            raise ValueError(f"Chunk-addressed binding differs: {binding_path}")
        ...
        for condition in evaluation.CONDITIONS:
            ...
            if not path.is_file():
                raise ValueError(f"Missing chunk-addressed predictions: {path}")
            rows = 0
            for raw_line in path.read_text(encoding="utf-8").splitlines():
                ...
                if any(record.get(key) != value for key, value in required.items()):
                    raise ValueError(
                        f"Chunk-addressed prediction binding differs: "
                        f"{path}:{source_index}"
                    )
                if source_index in by_condition[condition]:
                    raise ValueError(
                        f"Duplicate chunk-addressed prediction: "
    ...
    if len(expected_indices) != evaluation.EVALUATION_ROWS:
        raise ValueError("Chunk-addressed correct-state row count differs")
    for condition, records in by_condition.items():
        if set(records) != expected_indices:
            raise ValueError(f"Chunk-addressed condition row set differs: {condition}")
...
def analyze(root: Path) -> Mapping[str, Any]:
    ...
    gates = {
        ...
        "chunk_slots_aligned_on_every_generated_row": True,
        "projected_values_zero_on_every_generated_row": True,
    }
    ...
    status = (
        ...
        if native_gain
        else "chunk_addressed_value_native_gain_not_established"
    )
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "status": status,
...
def write_result(path: Path, result: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Chunk-addressed result output must be fresh: {path}")
    path.write_text(
