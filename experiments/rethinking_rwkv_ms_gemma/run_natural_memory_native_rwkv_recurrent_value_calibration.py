#!/usr/bin/env python3
"""Run one-update calibration for internally routed RWKV values."""
...
SCHEMA = "rwkv_ms_natural_memory_native_recurrent_value_calibration.v1"
PROTOCOL = (
    SCRIPT_DIR
    / "natural_memory_native_rwkv_recurrent_value_calibration_protocol_v1.json"
)
PROTOCOL_PAYLOAD_SHA256 = (
    "57d8ba4f2b1010afecc969d452cda706d29b24c73a6de9836f2e67df9b94f12a"
)
SCREEN_RESULT = (
    SCRIPT_DIR
...
SCREEN_RESULT_RECEIPT = (
    "6c13b95db2edd63c6cf47e9da38973a8e84ca98e5f0b55c6e38cabb4b70453b3"
)
SELECTED_CANDIDATE = {
...
def write_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Recurrent-value calibration output must be fresh: {path}")
...
def validate_protocol() -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, Mapping):
        raise ValueError("Recurrent-value calibration protocol receipt is missing")
    ...
    if protocol.get("authorization_basis") != required_authorization:
        raise ValueError("Recurrent-value calibration authorization differs")
    required_training = {
    ...
    if any(training.get(key) != value for key, value in required_training.items()):
        raise ValueError("Recurrent-value calibration training contract differs")
    if protocol.get("protected_splits_opened_by_this_protocol") != []:
        raise ValueError("Recurrent-value calibration may not open protected data")
    return protocol


def validate_screen_result() -> Mapping[str, Any]:
    if sha256_file(SCREEN_RESULT) != SCREEN_RESULT_FILE_SHA256:
        raise ValueError("Recurrent-value screen result file hash differs")
    result = json.loads(SCREEN_RESULT.read_text(encoding="utf-8"))
    receipt = result.get("receipt")
    if not isinstance(receipt, Mapping):
        raise ValueError("Recurrent-value screen result receipt is missing")
...
def run(
    *,
    ...
    if context.world_size != WORLD_SIZE:
        raise ValueError("Recurrent-value calibration requires exactly four ranks")
    ...
    if context.is_primary and resolved_output.exists():
        freshness_error = ValueError(
            f"Recurrent-value calibration output must be fresh: {resolved_output}"
        )
    distributed.phase_consensus(
        context,
        phase="recurrent-value-calibration-output-freshness",
    ...
    distributed.phase_consensus(
        context,
        phase="recurrent-value-calibration-output-creation",
    ...
    distributed.require_consensus(
        ...
        description="recurrent-value calibration initial adapter",
    )
    distributed.require_consensus(
        context,
        initial_recurrent_readout_sha256,
        description="recurrent-value calibration initial recurrent readout",
    )
    ...
    if global_answer_tokens <= 0:
        raise RuntimeError("Recurrent-value answer-token count is invalid")
    ...
    if not bool(torch.isfinite(gradient_norm).item()) or gradient_norm_value <= 0.0:
        raise RuntimeError("Recurrent-value gradient norm is invalid")
    optimizer.step()
    ...
    distributed.require_consensus(
        ...
        description="recurrent-value calibration updated adapter",
    )
    distributed.require_consensus(
        context,
        post_update_recurrent_readout_sha256,
        description="recurrent-value calibration updated recurrent readout",
    )
    ...
    states = {
        ...
        "zero_projected_bundle": screen.zero_projected_bundle(correct_state),
    ...
    checks = {
        ...
        "correct_exactly_equals_zero_projected_bundle_on_all_ranks": all(
            rank["post_update_candidate"]["checks"][
                "correct_exactly_equals_zero_projected_bundle"
            ]
            for rank in rank_evidence
        ),
        ...
        "all_post_update_logits_finite_on_all_ranks": all(
            rank["post_update_candidate"]["checks"]["all_condition_logits_finite"]
            for rank in rank_evidence
        ),
    }
    ...
    distributed.phase_consensus(
        context,
        phase="recurrent-value-calibration-result-save",
        error=save_error,
    )
    del model, optimizer, logits, batch, donor_batch, rows
    gc.collect()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if context is None:
        raise ValueError("Recurrent-value calibration requires four-rank torchrun")
    try:
        result = run(
            context=context,
