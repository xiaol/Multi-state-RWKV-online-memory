#!/usr/bin/env python3
"""Aggregate and sign the addressed-affine native generation benchmark."""
...
SCHEMA = "rwkv_ms_natural_memory_native_addressed_affine_result.v1"
MARGIN_MINIMUM = 0.005
COVERAGE_MINIMUM = 0.95
PARTITIONS_PER_SHARD = 1
PASS_STATUS = "addressed_affine_native_recurrent_causal_gain_established"
PARTIAL_STATUS = "addressed_affine_native_gain_without_full_causal_pass"
FAIL_STATUS = "addressed_affine_native_gain_not_established"
...
def write_result(path: Path, result: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Addressed-affine result output must be fresh: {path}")
    path.write_text(
