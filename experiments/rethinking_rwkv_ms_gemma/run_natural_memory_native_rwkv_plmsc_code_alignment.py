#!/usr/bin/env python3
"""Run the signed exact-v5 PLMSC write/read code-alignment screen."""
...
SCHEMA = "rwkv_ms_natural_memory_native_plmsc_code_alignment.v1"
ROW_SCHEMA = "rwkv_ms_natural_memory_native_plmsc_code_alignment_feature.v1"
SHARD_SCHEMA = "rwkv_ms_natural_memory_native_plmsc_code_alignment_shard.v1"
FEATURE_ROW_KEYS = {
    "schema",
    "capture_rank",
    "source_index",
...
PROTOCOL = (
    SCRIPT_DIR / "natural_memory_native_rwkv_plmsc_code_alignment_protocol_v1.json"
)
PROTOCOL_PAYLOAD_SHA256 = "5f849ec1c6fbc590f4bb8df6c976136213c928dc3270acc9e2291ec88ca76400"
PROTOCOL_FILE_SHA256 = "8428742275084b901d677395514228e2410323322d5041d92af6b6578f0cb93d"
PROTOCOL_SCHEMA = "rwkv_ms_natural_memory_native_plmsc_code_alignment_protocol.v1"
WORLD_SIZE = 4
...
def _capture_read_basis(
    ...
    if capture_index is not None:
        ...
        if selected.size(-1) != STATE_WIDTH:
            raise RuntimeError("PLMSC selected RWKV receptance width differs")
        module.rwkv_plmsc_read_basis_calls += 1
        module.rwkv_plmsc_prompt_boundary_r_seq = selected.detach().clone()
    return result
...
def install_anchor_read_capture(model: torch.nn.Module) -> Mapping[str, Any]:
    ...
    for module_name, module in causal_train.ordered_modules(model):
        ...
        module.rwkv_plmsc_prompt_boundary_r_seq = None
    ...
    *,
    ...
    try:
        ...
        for layer in ANCHORS:
            ...
            receptance = module.rwkv_plmsc_prompt_boundary_r_seq
            calls = int(module.rwkv_plmsc_read_basis_calls)
            read_basis_calls.append(calls)
            if calls != 1:
                raise RuntimeError(
                    "PLMSC anchor read-basis observer must run exactly once per read"
                )
            if receptance is None or tuple(receptance.shape) != (1, STATE_WIDTH):
                raise RuntimeError("PLMSC single-position RWKV read-basis capture differs")
            query_vectors.append(receptance[0].float().detach().clone())
        del logits
...
def _validate_feature_row(
    ...
    if (
        ...
        or row.get("read_basis_calls_per_anchor") != [1] * len(ANCHORS)
