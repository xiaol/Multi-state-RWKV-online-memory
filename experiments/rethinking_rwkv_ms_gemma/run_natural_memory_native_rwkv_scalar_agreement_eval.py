#!/usr/bin/env python3
"""Evaluate scalar-agreement RWKV control against a fixed projected carrier."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_rwkv_scalar_agreement_causal_train as training,
)


SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_eval.v1"
INPUT_SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_eval_input.v1"
PROTOCOL = (
    SCRIPT_DIR
    / "natural_memory_native_rwkv_scalar_agreement_generation_protocol_v1.json"
)
PROTOCOL_PAYLOAD_SHA256 = (
    "783c303a05c387e9cde67b3f37cfa50c34921239584e7de0eb8e9ca30a55395a"
)
TRAINING_PROTOCOL_PAYLOAD_SHA256 = training.PROTOCOL_PAYLOAD_SHA256
TRAIN_RESULT_FILE_SHA256 = (
    "33bdb546f0e788975ebe85a482465241c8b5f1f5da8854d30cc4960c9b545558"
)
TRAIN_RESULT_RECEIPT = (
    "b504fd3adef50ed80ccc3973a370fe67c8b0c90437348ec824da31dbde373002"
)
ADAPTER_CONFIG_SHA256 = (
    "542a6993983cc03011483b30505ac5ccbb74024fdc443737e79ef02e43d7640e"
)
...
def validate_protocol() -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, Mapping):
        raise ValueError("Scalar-agreement generation protocol receipt is missing")
    ...
    if (
        ...
        raise ValueError("Scalar-agreement generation protocol differs")
    ...
    if (
        ...
        raise ValueError("Scalar-agreement generation authorization differs")
    return protocol
...
def validate_train_result(
    ...
    *,
    ...
    if sha256_file(result_path) != TRAIN_RESULT_FILE_SHA256:
        raise ValueError("Scalar-agreement training result file differs")
    result = json.loads(result_path.read_text(encoding="utf-8"))
    receipt = result.get("receipt")
    if not isinstance(receipt, Mapping):
        raise ValueError("Scalar-agreement training result receipt is missing")
    unsigned = dict(result)
    unsigned.pop("receipt")
    required = {
        "schema": training.SCHEMA,
        "status": "scalar_agreement_heldout_passed_generation_authorized",
        "passed": True,
    ...
    if (
        ...
        raise ValueError("Scalar-agreement training did not authorize generation")
    ...
    if result.get("code_bindings", {}).get("runner_sha256") != sha256_file(
        ...
        raise ValueError("Scalar-agreement training runner binding differs")
    return result
...
def _restore_and_assert_scalar_agreement_modules(model) -> None:
    modules = tuple(iter_delta_mem_modules(model))
    if len(modules) != training.shared.preflight.EXPECTED_LAYERS:
        raise ValueError(f"Expected 42 scalar-agreement layers, found {len(modules)}")
    ...
    if not all(
        ...
        raise ValueError("Loaded adapter is not the locked scalar-agreement hybrid")
...
def generate_row_conditions(
    ...
    *,
    ...
    _restore_and_assert_scalar_agreement_modules(model)
...
def input_binding(
    *,
    ...
    return {
        ...
        "generation_batch_shape_control": "four_by_four_same_position",
        "projected_carrier_active": True,
        "rwkv_recurrent_scalar_controller_active": True,
