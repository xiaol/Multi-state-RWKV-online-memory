#!/usr/bin/env python3
"""Train a projected-carrier scalar agreement hybrid with causal controls."""
...
SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_causal_train.v1"
STEP_SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_causal_train_step.v1"
INPUT_SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_causal_train_input.v1"
PROTOCOL = (
    SCRIPT_DIR / "natural_memory_native_rwkv_scalar_agreement_causal_train_protocol_v1.json"
)
PROTOCOL_PAYLOAD_SHA256 = (
    "9d994b4b5e1faf00eb0a51a6961a8ffec945e7e0d63be4799efc947230539864"
)
SEED = positive.SEED
MIN_ACCEPTED_ROWS_PER_UPDATE = 6
MAX_TOTAL_REJECTED_ROWS = 8
SELECTED_CANDIDATE = {
    "candidate_id": "scalar_agreement_t16_k2_gate025_g003125",
...
FILTERED_CONTRAST_RESULT = (
    SCRIPT_DIR
    / "local_artifacts/"
    "natural_memory_native_rwkv_filtered_contrast_causal_train_v1/result.json"
)
FILTERED_CONTRAST_RESULT_FILE_SHA256 = (
    "43124c1478aa9f4aad2c0f93fc77d116829ca9a4fda2406194d1ab28ab09f3bd"
)
FILTERED_CONTRAST_RESULT_RECEIPT = (
    "79ec7d7aa47c968337268ca60df87f21e560d5ce19acb1e740c06197b85b5eff"
)
...
def load_model(
    ...
    *,
    ...
    configured = all(
        module.rwkv_ms_hybrid_mode == "scalar_gate"
    ...
    audit = {
        **dict(inherited_audit),
        "all_wrappers_scalar_agreement_content_gated": configured,
    }
    if not configured:
        raise RuntimeError(f"Scalar-agreement attachment failed: {audit!r}")
...
def validate_protocol() -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, Mapping):
        raise ValueError("Scalar-agreement protocol receipt is missing")
    unsigned = dict(protocol)
    unsigned.pop("receipt")
    digest = shared.canonical_sha256(unsigned)
    if digest != PROTOCOL_PAYLOAD_SHA256 or receipt.get("payload_sha256") != digest:
        raise ValueError("Scalar-agreement protocol payload differs")
    if shared.sha256_file(FILTERED_CONTRAST_RESULT) != (
        FILTERED_CONTRAST_RESULT_FILE_SHA256
    ):
        raise ValueError("Filtered-contrast result binding differs")
    prior = json.loads(FILTERED_CONTRAST_RESULT.read_text(encoding="utf-8"))
    unsigned_prior = dict(prior)
    prior_receipt = unsigned_prior.pop("receipt", {})
    if (
        shared.canonical_sha256(unsigned_prior) != FILTERED_CONTRAST_RESULT_RECEIPT
        or prior_receipt.get("payload_sha256") != FILTERED_CONTRAST_RESULT_RECEIPT
        or prior.get("status") != "filtered_contrast_heldout_failed_generation_blocked"
        or prior.get("training_passed") is not True
        or prior.get("passed") is not False
    ):
        raise ValueError("Filtered-contrast failure does not authorize training")
    ...
    if (
        ...
        raise ValueError("Scalar-agreement training contract differs")
    if protocol.get("protected_splits_opened_by_this_protocol") != []:
        raise ValueError("Scalar-agreement training may not open protected data")
    return protocol
...
def training_bindings() -> Iterator[None]:
    with positive.training_bindings():
        ...
        shared_bindings = {
            ...
            "PASS_STATUS": "scalar_agreement_heldout_passed_generation_authorized",
            "FAIL_STATUS": "scalar_agreement_heldout_failed_generation_blocked",
