#!/usr/bin/env python3
"""Run the exact-Value14-only Scene Memory V14 candidate gate."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    scene_memory_v14_warm_start as v14_warm,
)
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    scene_memory_v13_warm_start as v13_warm,
)
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
...
GATE_CONTRACT = "scene_memory_v14_exact_value14_candidate_gate"
GATE_RECORD_SCHEMA = "rwkv_ms_scene_memory_v14_value14_gate_record.v1"
GATE_SUMMARY_SCHEMA = "rwkv_ms_scene_memory_v14_value14_gate_summary.v1"
GATE_MANIFEST_SCHEMA = "rwkv_ms_scene_memory_v14_value14_gate_manifest.v1"
GATE_RECEIPT_SCHEMA = "rwkv_ms_scene_memory_v14_value14_gate_receipt.v1"
CANDIDATE_DESIGNATION_KIND = "scene_memory_v14_value14_candidate_receipt"
...
BENCHMARK_LOCK = v10.v9.v8.BENCHMARK_LOCK
HARD32_AUTHORIZATION_KIND = "scene_memory_v14_value14_gate_receipt"
HARD32_AUTHORIZATION_SCOPE = v10.v9.v8.HARD32_AUTHORIZATION_SCOPE
...
V14_OBJECTIVE = {
    **v10.V10_OBJECTIVE,
    ...
    "progression_basis": "four_cycle_value14_cached_prefix_boundary_fit_v1",
    "cached_prefix_mode": launch.CACHED_PREFIX_MODE,
    "failed_replay_mode": launch.FAILED_REPLAY_MODE,
    "exact_replay_mode": launch.EXACT_REPLAY_MODE,
    "exact_retention_scope": launch.EXACT_RETENTION_SCOPE,
    "exact_retention_hinge_mode": launch.EXACT_RETENTION_HINGE_MODE,
    "cached_prefix_margin": launch.SEMANTIC_MARGIN,
    "cached_replay_use_cache": True,
    "cached_replay_logits_to_keep": launch.REPLAY_LOGITS_TO_KEEP,
    ...
    "warm_start_source": "pinned_v13_checkpoint4_adapter_only",
...
class V14EvaluationContractError(ValueError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise V14EvaluationContractError(message)
...
def _run_path(path: Path | str, *, description: str, ssd_root: Path) -> Path:
    try:
        return launch.require_v14_run_path(
            path,
            description=description,
            ssd_root=ssd_root,
        )
    except Exception as exc:
        raise V14EvaluationContractError(
            f"V14 {description} must stay under the locked V14 run root: {exc}"
        ) from exc


def _gate_path(path: Path | str, *, description: str, ssd_root: Path) -> Path:
    try:
        return launch.require_v14_gate_path(
            path,
            description=description,
            ssd_root=ssd_root,
        )
        ...
        raise V14EvaluationContractError(
            f"V14 {description} must stay under the locked V14 gates root: {exc}"
        ) from exc
    ...
    *,
    ...
    try:
        _require(
            ...
            "V14 pinned base model override is forbidden",
        )
        ...
        raise V14EvaluationContractError(
            "V14 base model must equal the model and prompt artifacts in the "
...
def _regular_file(
    ...
    *,
    ...
    _require(resolved.is_file(), f"V14 {description} is missing: {resolved}")
    return resolved
...
def _load_json(path: Path | str, *, description: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise V14EvaluationContractError(f"V14 {description} is invalid JSON") from exc
    _require(isinstance(payload, dict), f"V14 {description} must be an object")
    return payload
...
def _read_jsonl(path: Path, *, description: str) -> list[dict[str, Any]]:
    ...
    for row_number, line in enumerate(
        ...
        _require(bool(line.strip()), f"V14 {description} contains a blank row")
        try:
            payload = json.loads(line)
        except json.JSONDecodeError as exc:
            raise V14EvaluationContractError(
                f"V14 {description} row {row_number} is invalid JSON"
            ) from exc
        _require(isinstance(payload, dict), f"V14 {description} row must be an object")
...
def _validate_self_hash(payload: Mapping[str, Any], *, field: str) -> str:
    ...
    _require(isinstance(recorded, str), f"V14 {field} is missing")
    _require(
...
def validate_v14_train_inputs(
    *,
    ...
    try:
        ...
        baseline = launch.validate_v10_diagnostic_baseline(ssd_root=ssd_root)
        benchmark = v10.v9.v8.validate_benchmark_lock(benchmark_lock_path)
    except Exception as exc:
        raise V14EvaluationContractError(f"V14 input contract failed: {exc}") from exc
    _require(
        launch_data["four_cycle_pairs"]
        == [list(pair) for pair in launch.FOUR_CYCLE_PAIRS]
        and launch_data["four_cycle_pairs_sha256"]
        == launch.FOUR_CYCLE_PAIRS_SHA256,
        "V14 four-cycle identity differs",
    )
    result = dict(base)
    result.update(
        {
            ...
            "checkpoint_steps": list(launch.CHECKPOINT_STEPS),
            "presentation_checkpoint_steps": list(
                launch.PRESENTATION_CHECKPOINTS
            ),
...
def _validate_v14_objective_protocol(
    ...
    *,
    ...
    _require(
        ...
        "V14 objective source identity differs",
    )


def validate_v14_checkpoint(
    memory_dir: Path | str,
    *,
    input_contract: Mapping[str, Any],
    ...
    try:
        ...
        raise V14EvaluationContractError(f"V14 checkpoint contract failed: {exc}") from exc
    resolved = Path(str(lineage["checkpoint"]))
    checkpoint_step = int(lineage["checkpoint_step"])
    protocol = _load_json(
        resolved / "training_protocol.json",
        description="V14 training protocol",
    )
    _validate_v14_objective_protocol(protocol, input_contract=input_contract)
    pairing = _load_json(
        resolved / "scene_state_identity_pairing_manifest.json",
        description="V14 checkpoint pairing manifest",
    )
    try:
        ...
        raise V14EvaluationContractError(f"V14 pairing contract failed: {exc}") from exc
    ...
    _require(
        ...
        "V14 checkpoint architecture differs",
    )
    artifacts = {
        name.removesuffix(".json").removesuffix(".pt").removesuffix(".pth"): (
            _artifact_binding(resolved / name, description=f"V14 checkpoint {name}")
        )
        for name in launch.REQUIRED_CHECKPOINT_ARTIFACTS
    }
    rng = [
        _artifact_binding(path, description=f"V14 checkpoint RNG {path.name}")
    ...
    checkpoint_contract = {
        ...
        "lineage_artifact": _artifact_binding(
            lineage_path,
            description="V14 checkpoint lineage",
        ),
        "architecture": architecture,
        "objective": dict(V14_OBJECTIVE),
    ...
    try:
        ...
        raise V14EvaluationContractError(
            f"V14 training provenance failed: {exc}"
        ) from exc
...
def build_v14_gate(
    *,
    ...
    _require(
        ...
        "V14 gate accepts only a locked checkpoint from 1 through 4",
    )
    _require(
        set(records_by_condition) == set(CONDITIONS),
        "V14 gate conditions differ",
    )
    ...
    for condition in CONDITIONS:
        ...
        _require(
            ...
            f"V14 gate requires exactly 14 {condition} rows",
        )
        ...
        for expected_ordinal, record in zip(VALUE14_ORDINALS, records, strict=True):
            ...
            _require(
                ...
                "V14 gate rows must be exactly Value14 in canonical order",
            )
            _require(record.get("condition") == condition, "V14 gate condition differs")
    ...
    _require(
        ...
        "V14 Value14 donor mapping differs",
    )
    scores = {
    ...
    correct_donor_semantic_switch_rows = sum(
        v10.v9.v8._strict_exact(scores[ordinal])
        and v10.v9.v8._strict_exact(donor_identity_scores[ordinal])
        and not v10.v9.v8._strict_exact(donor_current_scores[ordinal])
        for ordinal in VALUE14_ORDINALS
    )
    generation = {
        "rows": 14,
        ...
        "correct_donor_semantic_switch_rows": (
            correct_donor_semantic_switch_rows
        ),
    ...
    gates = {
        "value14_correct_semantic_exact": (
            generation["correct_strict_exact_rows"]
            == GATE_REQUIREMENTS["correct_strict_exact_rows"]
        ),
        "value14_donor_semantic_exact": (
            generation["donor_identity_strict_exact_rows"]
            == GATE_REQUIREMENTS["donor_identity_strict_exact_rows"]
        ),
        "value14_correct_donor_semantic_switch": (
            generation["correct_donor_semantic_switch_rows"]
            == GATE_REQUIREMENTS["correct_donor_semantic_switch_rows"]
        ),
        "value14_zero_does_not_reproduce_correct_memory": (
            generation["zero_strict_exact_rows"]
            <= GATE_REQUIREMENTS["zero_strict_exact_rows_max"]
        ),
    ...
    return {
            ...
            "criterion": "value14_semantic_boundary_in_sample_memory_fit_v2",
...
def validate_resume_records(
    ...
    *,
    ...
    _require(condition in CONDITIONS, "V14 resume condition differs")
    _require(
        len(records) <= len(VALUE14_ORDINALS),
        "V14 resume records exceed Value14",
    )
    validated: dict[int, dict[str, Any]] = {}
    for position, record in enumerate(records):
        current = dict(record)
        _require(current.get("schema") == GATE_RECORD_SCHEMA, "V14 record schema differs")
        _validate_self_hash(current, field="record_sha256")
        ordinal = VALUE14_ORDINALS[position]
        _require(current.get("fingerprint") == fingerprint, "V14 record fingerprint differs")
        _require(current.get("condition") == condition, "V14 record condition differs")
        _require(current.get("split") == "train", "V14 record split differs")
        _require(
            current.get("train_row_ordinal") == ordinal,
            "V14 records are not a canonical Value14 prefix",
        )
        sample = rows[ordinal]
        expected_donor = donor_by_ordinal[ordinal]
        _require(current.get("source_index") == sample["source_index"], "V14 source differs")
        _require(current.get("row_sha256") == sample["row_sha256"], "V14 row hash differs")
        _require(current.get("gold") == sample["gold"], "V14 record gold differs")
        _require(
            current.get("donor_train_row_ordinal") == expected_donor
            and current.get("donor_source_index")
            == rows[expected_donor]["source_index"],
            "V14 donor identity differs",
        )
        raw_generation = current.get("raw_generation")
        _require(isinstance(raw_generation, str), "V14 raw generation differs")
        _require(
            v10.v9.v8.extract_json(raw_generation) == current.get("parsed_json"),
            "V14 parsed JSON differs",
        )
...
def evaluator_code_binding() -> dict[str, Any]:
    paths = {
        "v14_gate": Path(__file__).resolve(),
        ...
        "v14_launch_contract": Path(launch.__file__).resolve(),
        "v14_warm_start": Path(v14_warm.__file__).resolve(),
        "v13_launch_contract": Path(launch.v13.__file__).resolve(),
        "v10_launch_contract": Path(launch.v13.v10.__file__).resolve(),
    ...
    return {
        name: _artifact_binding(path, description=f"V14 evaluator code {name}")
...
def build_evaluation_fingerprint_payload(
    *,
    ...
    _require(
        ...
        "V14 fingerprint checkpoint layer identity differs",
    )
    _require(
        isinstance(checkpoint.get("training_provenance"), Mapping),
        "V14 fingerprint training provenance is missing",
    )
    return {
        "schema_version": 1,
        "contract": GATE_CONTRACT,
...
def validate_existing_manifest(
    ...
    *,
    ...
    _require(
        ...
        "V14 existing manifest fields differ",
    )
    if require_postload:
        _require(set(manifest) == postload_keys, "V14 completed manifest lacks runtime identity")
    _require(manifest.get("schema") == GATE_MANIFEST_SCHEMA, "V14 manifest schema differs")
    _require(
        isinstance(manifest.get("created_at"), str) and bool(manifest["created_at"]),
        "V14 manifest creation time differs",
    )
    expected_payload = dict(expected_fingerprint_payload)
    _require(
        manifest.get("fingerprint_payload") == expected_payload,
        "V14 manifest fingerprint payload differs",
    )
    _require(
        manifest.get("fingerprint") == expected_fingerprint
        and expected_fingerprint
        == v10.v9.fingerprint_payload_sha256(expected_payload),
        "V14 manifest fingerprint differs",
    )
    _require(
        manifest.get("hard32_access") == HARD32_ACCESS_POLICY,
        "V14 manifest Hard32 identity differs",
    )
    if "runtime_prefixes" in manifest:
        _require(
            ...
            "V14 manifest runtime identity differs",
        )
...
def bind_or_validate_manifest_runtime(
    ...
    *,
    ...
    _require(has_prefixes == has_profile, "V14 manifest runtime fields are incomplete")
    if has_prefixes:
        _require(bound["runtime_prefixes"] == runtime_prefixes, "V14 runtime prefixes differ")
        _require(
            bound["runtime_fusion_profile"] == runtime_fusion_profile,
            "V14 runtime fusion profile differs",
        )
        return bound
...
def build_gate_receipt(
    *,
    ...
    output_dir = _gate_path(
        output_dir,
        description="V14 gate output directory",
        ssd_root=ssd_root,
    )
    outputs = {
        "manifest": _artifact_binding(
            output_dir / "manifest.json",
            description="V14 gate manifest",
        ),
        "summary": _artifact_binding(
            output_dir / "summary.json",
            description="V14 gate summary",
        ),
        "conditions": {
            condition: _artifact_binding(
                output_dir / f"{condition}.jsonl",
                description=f"V14 {condition}",
            )
            for condition in CONDITIONS
        },
    }
...
def _verify_artifact_binding(
    ...
    *,
    ...
    _require(isinstance(binding, Mapping), f"V14 {description} binding is missing")
    raw_path = binding.get("path")
    _require(
        isinstance(raw_path, str) and bool(raw_path),
        f"V14 {description} path differs",
    )
    expected = _gate_path(
        raw_path,
        description=description,
    ...
    try:
        ...
        raise V14EvaluationContractError(f"V14 {description} differs: {exc}") from exc
    _require(actual == expected, f"V14 {description} resolved path differs")
    return actual
    ...
    *,
    ...
    if isinstance(receipt, Mapping):
        ...
        receipt_path = _regular_file(
            receipt,
            description="V14 gate receipt",
            ssd_root=ssd_root,
        )
        payload = _load_json(receipt_path, description="V14 gate receipt")
    _require(payload.get("schema") == GATE_RECEIPT_SCHEMA, "V14 receipt schema differs")
    _validate_self_hash(payload, field="receipt_sha256")
    _require(payload.get("contract") == GATE_CONTRACT, "V14 receipt contract differs")
    _require(payload.get("objective") == V14_OBJECTIVE, "V14 receipt objective differs")
    _require(
        payload.get("requirements") == GATE_REQUIREMENTS,
        "V14 receipt requirements differ",
    )
    inputs = (
        validate_v14_train_inputs(ssd_root=ssd_root)
        if input_contract is None
        else dict(input_contract)
    )
    _require(
        launch_receipt is not None and completion_receipt is not None,
        "V14 receipt replay requires exact launch and completion receipts",
    )
    checkpoint = validate_v14_checkpoint(
        memory_dir,
    ...
    _require(payload.get("checkpoint") == checkpoint, "V14 receipt checkpoint differs")
    _require(
        payload.get("training_provenance") == checkpoint["training_provenance"],
        "V14 receipt training provenance differs",
    )
    _require(
        payload.get("training_sources") == inputs["artifacts"],
        "V14 receipt sources differ",
    )
    _require(
        payload.get("v10_diagnostic_baseline")
        == inputs["v10_diagnostic_baseline"],
        "V14 receipt V10 diagnostic differs",
    )
    ...
    _require(
        ...
        "V14 receipt evaluation fingerprint differs from live inputs",
    )
    outputs = payload.get("outputs")
    _require(isinstance(outputs, Mapping), "V14 receipt outputs missing")
    manifest_path = _verify_artifact_binding(
        outputs.get("manifest", {}),
        description="V14 receipt manifest",
        ssd_root=ssd_root,
    )
    summary_path = _verify_artifact_binding(
        outputs.get("summary", {}),
        description="V14 receipt summary",
        ssd_root=ssd_root,
    )
    manifest = validate_existing_manifest(
        _load_json(manifest_path, description="V14 receipt manifest"),
        expected_fingerprint=fingerprint,
        expected_fingerprint_payload=fingerprint_payload,
        require_postload=True,
    )
    summary = _load_json(summary_path, description="V14 receipt summary")
    _require(summary.get("schema") == GATE_SUMMARY_SCHEMA, "V14 summary schema differs")
    _validate_self_hash(summary, field="summary_sha256")
    _require(
        manifest.get("fingerprint") == fingerprint
        and summary.get("fingerprint") == fingerprint,
        "V14 output fingerprints differ",
    )
    bindings = outputs.get("conditions")
    _require(
        isinstance(bindings, Mapping) and set(bindings) == set(CONDITIONS),
        "V14 condition bindings differ",
    )
    ...
    for condition in CONDITIONS:
        ...
        indexed = validate_resume_records(
            _read_jsonl(path, description=f"V14 {condition}"),
            condition=condition,
        ...
        _require(
            ...
            f"V14 {condition} output must contain exactly Value14",
        )
        records[condition] = [indexed[index] for index in VALUE14_ORDINALS]
    recomputed = build_v14_gate(
    ...
    _require(
        ...
        "V14 gate does not reproduce",
    )
    _require(payload.get("code") == evaluator_code_binding(), "V14 evaluator code differs")
    ...
    _require(
        ...
        "V14 candidate designation differs",
    )
    _require(
        ...
        and authorization.get("hard32_authorized") is passed
        ...
        "V14 receipt training authorization differs",
    )
    result = dict(payload)
...
def validate_continuation_authorization(*args: Any, **kwargs: Any) -> dict[str, Any]:
    raise V14EvaluationContractError(
        "V14 is a terminal fresh four-cycle run; no gate authorizes continuation"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _require(args.max_new_tokens == GATE_MAX_NEW_TOKENS, "V14 gate requires 128 tokens")
    _require(
        args.expected_memory_layer_count == GATE_EXPECTED_MEMORY_LAYER_COUNT,
        "V14 gate requires all 42 layers",
    )
    _require(args.device == GATE_DEVICE, "V14 gate device differs")
    _require(args.dtype == GATE_DTYPE, "V14 gate dtype differs")
    _require(
        args.attn_implementation == GATE_ATTN_IMPLEMENTATION,
        "V14 gate attention implementation differs",
    )
    _require(
        args.normal_fusion_profile == GATE_NORMAL_FUSION_PROFILE,
        "V14 gate fusion profile differs",
    )
    args.delta_mem_root = str(Path(args.delta_mem_root).expanduser().resolve())
    _require(Path(args.delta_mem_root) == PROJECT_ROOT, "V14 gate requires this checkout")
    ...
    inputs = validate_v14_train_inputs()
    ...
    _require(expected_layers == 42, "V14 resolved memory layer count differs")
    fingerprint_payload = build_evaluation_fingerprint_payload(
        input_contract=inputs,
        checkpoint=checkpoint,
    ...
    if paths["manifest"].exists():
        manifest = validate_existing_manifest(
            _load_json(paths["manifest"], description="existing V14 manifest"),
            expected_fingerprint=fingerprint,
            expected_fingerprint_payload=fingerprint_payload,
        )
    else:
    ...
    for condition in CONDITIONS:
        existing = (
            _read_jsonl(paths[condition], description=f"V14 {condition}")
