#!/usr/bin/env python3
"""Analyze and sign the adaptive dual-path repair candidate."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_scene_dualpath_repair_eval as runner,
)


SCHEMA = "rwkv_ms_natural_memory_native_scene_dualpath_repair_result.v1"
...
def analyze(**kwargs: Any) -> Mapping[str, Any]:
    ...
    if result["status"] == "passed":
        result["decision"] = (
            "Adaptive TRAIN-only dual-path repair cleared the development gates, "
            "but benchmark-informed method design forbids protected-split authorization."
        )
    else:
        result["decision"] = (
            "Adaptive TRAIN-only dual-path repair failed at least one development gate; "
