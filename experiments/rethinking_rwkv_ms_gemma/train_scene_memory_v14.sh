...
RUN_ROOT="${SSD_ROOT}/delta_mem_outputs/novel_rwkv_ms_memory/scene_memory_v14"
GATES_ROOT="${RUN_ROOT}/gates"
CACHE_ROOT="${SSD_ROOT}/delta_mem_cache/scene_memory_v14"
...
if [[ "${SMOKE_RUN}" == "1" ]]; then
  [[ "${TARGET_STEP}" == "1" ]] || fail "v14_smoke_target_step_must_be_one"
else
  [[ "${TARGET_STEP}" == "4" ]] || fail "v14_target_step_must_be_four"
fi
[[ -z "${RESUME_FROM_CHECKPOINT}" ]] || fail "v14_resume_is_forbidden"
[[ -z "${GATE_RECEIPT}" ]] || fail "v14_gate_receipt_cannot_authorize_training"
...
RUN_ID="scene_memory_v14_${run_kind}_${RUN_NAME}_step${TARGET_STEP}"
...
if [[ "${DRY_RUN}" == "0" ]]; then
  [[ -z "$(git -C "${REPO}" status --porcelain --untracked-files=no)" ]] \
    || fail "tracked_worktree_must_be_clean_before_v14_training"
fi
...
if [[ "${DRY_RUN}" == "0" ]]; then
  for critical_file in "${critical_tracked_files[@]}"; do
    git -C "${REPO}" ls-files --error-unmatch -- "${critical_file}" >/dev/null 2>&1 \
      || fail "critical_v14_source_must_be_tracked path=${critical_file}"
  done
fi
...
contract_args=(
  -m experiments.rethinking_rwkv_ms_gemma.scene_memory_v14_launch_contract
...
if ! contract_values="$("${VALIDATION_PYTHON_BIN}" "${contract_args[@]}")"; then
  fail "v14_launch_contract_validation_failed"
fi

IFS=$'\t' read -r \
  TRAIN_FILE TRAIN_SHA256 SOURCE_MANIFEST SOURCE_MANIFEST_SHA256 \
  SCHEDULE_FILE SCHEDULE_SHA256 WARM_START_CHECKPOINT \
  WARM_START_ADAPTER_SHA256 LAUNCH_MODE SOURCE_STEP \
  VALIDATED_TARGET_STEP RESUME_SCHEDULE_CURSOR \
  NEXT_PAIR_LOW_ORDINAL NEXT_PAIR_HIGH_ORDINAL NEXT_SCHEDULE_ENTRY_SHA256 \
  SAVE_STEPS GRADIENT_ACCUMULATION_STEPS MAX_STEPS SAVE_TOTAL_LIMIT \
  TOTAL_PAIR_PRESENTATIONS RUN_MODE PRODUCTION_ELIGIBLE \
  V10_DIAGNOSTIC_SUMMARY_SHA256 <<<"${contract_values}"
[[ "${VALIDATED_TARGET_STEP}" == "${TARGET_STEP}" ]] \
  || fail "validated_target_step_differs"
[[ "${SOURCE_STEP}" == "0" && "${RESUME_SCHEDULE_CURSOR}" == "0" ]] \
  || fail "v14_must_begin_fresh_at_zero"
...
train_args=(
  ...
  --scene-state-generation-objective-version scene_state_generation_ce_symmetric_cached_prefix_boundary_v14
...
if [[ "${SMOKE_RUN}" == "1" ]]; then
  train_args+=(--scene-state-v14-one-pair-smoke)
fi
train_command=("${PYTHON_BIN}" -m deltamem.train.delta_sft "${train_args[@]}")

printf 'Validated V14 mode=%s target_step=%s pair_presentations=%s; first pair=%s,%s\n' \
  "${RUN_MODE}" "${TARGET_STEP}" "${TOTAL_PAIR_PRESENTATIONS}" \
...
if [[ "${DRY_RUN}" == "1" ]]; then
  printf 'Validated V14 training command (not started):\n'
...
"${VALIDATION_PYTHON_BIN}" - \
  ...
  "${CODE_COMMIT}" "${SMOKE_RUN}" "${RUN_MODE}" "${TARGET_STEP}" \
  "${GRADIENT_ACCUMULATION_STEPS}" "${MAX_STEPS}" \
  "${SAVE_TOTAL_LIMIT}" "${TOTAL_PAIR_PRESENTATIONS}" \
  "${PRODUCTION_ELIGIBLE}" <<'PY'
...
(
    ...
    run_mode,
    target_step_raw,
    gradient_accumulation_raw,
    max_steps_raw,
    save_total_limit_raw,
    pair_presentations_raw,
    production_eligible_raw,
) = sys.argv[1:]
smoke = smoke_raw == "1"
target_step = int(target_step_raw)
gradient_accumulation_steps = int(gradient_accumulation_raw)
max_steps = int(max_steps_raw)
save_total_limit = int(save_total_limit_raw)
pair_presentations = int(pair_presentations_raw)
production_eligible = production_eligible_raw == "True"
trainer_output_path = Path(trainer_output).resolve()
checkpoint_steps = (
    launch.ONE_PAIR_SMOKE_CHECKPOINT_STEPS if smoke else launch.CHECKPOINT_STEPS
)
checkpoints = {
...
payload = {
    "schema": (
        launch.ONE_PAIR_SMOKE_LAUNCH_RECEIPT_SCHEMA
        if smoke
        else launch.LAUNCH_RECEIPT_SCHEMA
    ),
    ...
    "objective_schema_version": launch.OBJECTIVE_SCHEMA_VERSION,
    "gradient_accumulation_steps": gradient_accumulation_steps,
    "max_grad_norm": launch.MAX_GRAD_NORM,
    "max_steps": max_steps,
    "learning_rate": launch.LEARNING_RATE,
    "optim": launch.OPTIMIZER_IMPLEMENTATION,
    "weight_decay": launch.WEIGHT_DECAY,
    "lr_scheduler_type": "constant",
    "warmup_steps": launch.WARMUP_STEPS,
    "warmup_ratio": launch.WARMUP_RATIO,
    "logging_steps": launch.LOGGING_STEPS,
    "save_steps": launch.SAVE_STEPS,
    "save_total_limit": save_total_limit,
    "seed": launch.SEED,
    "data_seed": launch.DATA_SEED,
    "total_pair_presentations": pair_presentations,
    "scheduled_pairs": (
    ...
    "warm_start_adapter_sha256": launch.PINNED_WARM_START_ADAPTER_SHA256,
    "warm_start_mode": warm_contract["warm_start_mode"],
    "warm_start_lock": warm_contract["warm_start_lock"],
    "warm_start_lock_sha256": warm_contract["warm_start_lock_sha256"],
    ...
    "evaluation_access": "forbidden" if smoke else "value14_gate_only",
    "git_commit": commit,
}
...

{
  ...
  printf 'Starting attached V14 four-cycle training. Command: '
  printf '%q ' "${train_command[@]}"
  printf '\n'
} | tee "${LOG_FILE}"
...
set -e
(( pipeline_status[0] == 0 )) \
  || fail "v14_training_failed exit_code=${pipeline_status[0]} log=${LOG_FILE}"
(( pipeline_status[1] == 0 )) \
  || fail "v14_training_log_failed exit_code=${pipeline_status[1]}"
...
checkpoint_args=("${CHECKPOINT1_DIR}")
if [[ "${SMOKE_RUN}" == "0" ]]; then
  ...
  checkpoint_args+=("${CHECKPOINT2_DIR}" "${CHECKPOINT3_DIR}" "${CHECKPOINT4_DIR}")
fi

"${VALIDATION_PYTHON_BIN}" - \
  "${COMPLETION_RECEIPT}" "${LAUNCH_RECEIPT}" "${LOG_FILE}" \
  "${OUTPUT_DIR}/training_summary.json" "${SMOKE_RUN}" \
  "${checkpoint_args[@]}" <<'PY'
from datetime import datetime, timezone
import json
from pathlib import Path
...
output_raw, launch_raw, log_raw, summary_raw, smoke_raw, *checkpoint_raw = sys.argv[1:]
smoke = smoke_raw == "1"
...
if smoke:
    if len(checkpoints) != 1 or checkpoints[0].name != "checkpoint-1":
        raise SystemExit("V14 smoke completion requires only checkpoint-1")
    checkpoint_contracts = {
        checkpoints[0].name: launch.validate_one_pair_smoke_checkpoint_contract(
            checkpoints[0],
            data=data,
            warm=warm,
        )
    ...
    if [checkpoint.name for checkpoint in checkpoints] != [
        f"checkpoint-{step}" for step in launch.CHECKPOINT_STEPS
    ]:
        raise SystemExit("V14 production completion requires checkpoints 1 through 4")
    checkpoint_contracts = {
...
baseline = launch.validate_v10_diagnostic_baseline()
if smoke:
    launch_validation = launch.validate_one_pair_smoke_launch_receipt(
        Path(launch_raw),
...
payload = {
    "schema": (
        launch.ONE_PAIR_SMOKE_COMPLETION_RECEIPT_SCHEMA
        if smoke
        else launch.COMPLETION_RECEIPT_SCHEMA
    ),
    "completed_at": datetime.now(timezone.utc).isoformat(),
    "status": "completed",
    "optimizer_step": 1 if smoke else launch.TOTAL_OPTIMIZER_STEPS,
    "consumed_pair_presentations": 1 if smoke else launch.TOTAL_PAIR_PRESENTATIONS,
    "launch_receipt": launch_validation["artifact"],
    "launch_receipt_sha256": launch_validation["receipt_sha256"],
    "log": launch.artifact_binding(Path(log_raw), description="v14_completion_log"),
        ...
        checkpoint.name: {
            ...
            "checkpoint_artifacts": {
                name: launch.artifact_binding(
                    checkpoint / name,
                    description=f"v14_completion_{checkpoint.name}_{name}",
                )
            ...
            "rng_state_artifacts": {
                path.name: launch.artifact_binding(
                    path,
                    description=f"v14_completion_{checkpoint.name}_{path.name}",
                )
...
with Path(output_raw).open("x", encoding="utf-8") as handle:
    ...
    handle.write("\n")
if smoke:
    launch.validate_one_pair_smoke_completion_receipt(
        Path(output_raw),
        checkpoint=checkpoints[0],
        checkpoint_contract=checkpoint_contracts[checkpoints[0].name],
...
if [[ "${SMOKE_RUN}" == "1" ]]; then
  printf 'Completed V14 one-pair real-backward smoke at checkpoint-1: %s\n' "${OUTPUT_DIR}"
  printf 'Smoke is not production-eligible and cannot authorize Value14 or Hard32.\n'
else
  printf 'Completed attached V14 optimizer checkpoints 1 through 4: %s\n' "${OUTPUT_DIR}"
  printf 'Gate provenance: --launch-receipt %s --completion-receipt %s\n' \
    "${LAUNCH_RECEIPT}" "${COMPLETION_RECEIPT}"
  printf 'Gate Value14 in checkpoint order 1,2,3,4 and stop at the first pass; never resume this run.\n'
fi
