#!/usr/bin/env python3
"""V11 identity wrapper for the pinned V8 checkpoint-56 adapter warm start."""
...
WARM_START_MODE = "scene_memory_v11_v8_checkpoint56_adapter_only"
RECEIPT_SCHEMA = "rwkv_ms_scene_memory_v11_adapter_warm_start_receipt.v1"
...
def validate_v11_fresh_start_contract(
    contract: V11FreshStartContract,
) -> dict[str, Any]:
    if contract.resume_from_checkpoint is not None:
        raise ValueError("V11 is one-cycle fresh training and forbids checkpoint resume")
    if isinstance(contract.initial_global_step, bool) or contract.initial_global_step != 0:
        raise ValueError("V11 adapter warm start must begin at global step 0")
    imported = {
    ...
    if forbidden:
        raise ValueError(
            "V11 adapter warm start forbids preloaded training state: "
            + ", ".join(forbidden)
        )
    if contract.optim not in ALLOWED_FRESH_ADAMW_IMPLEMENTATIONS:
        raise ValueError(
            "V11 adapter warm start requires a fresh AdamW optimizer created after load"
        )
    return {
...
def apply_v11_v8_checkpoint56_adapter_only_warm_start(
    model: nn.Module,
    context: V11WarmStartContext,
    *,
    fresh_start: V11FreshStartContract,
) -> dict[str, Any]:
    fresh_start_receipt = validate_v11_fresh_start_contract(fresh_start)
    ...
    if unequal:
        raise ValueError(
            "V11 warm-start adapter tensors are not bit-equal after loading: "
            + ", ".join(unequal[:8])
        )
...
def write_v11_warm_start_receipt(
    ...
    if recorded_sha256 != v9.canonical_sha256(payload):
        raise ValueError("V11 warm-start receipt self-hash differs")
...
__all__ = (
    ...
    "validate_v11_fresh_start_contract",
    "write_v11_warm_start_receipt",
)
