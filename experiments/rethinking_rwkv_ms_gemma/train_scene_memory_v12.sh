...
RUN_ROOT="${SSD_ROOT}/delta_mem_outputs/novel_rwkv_ms_memory/scene_memory_v12"
GATES_ROOT="${RUN_ROOT}/gates"
CACHE_ROOT="${SSD_ROOT}/delta_mem_cache/scene_memory_v12"
...
[[ "${TARGET_STEP}" == "2" ]] || fail "v12_target_step_must_be_two"
[[ -z "${RESUME_FROM_CHECKPOINT}" ]] || fail "v12_resume_is_forbidden"
[[ -z "${GATE_RECEIPT}" ]] || fail "v12_gate_receipt_cannot_authorize_training"
...
RUN_ID="scene_memory_v12_${run_kind}_${RUN_NAME}_step2"
...
if [[ "${DRY_RUN}" == "0" ]]; then
  [[ -z "$(git -C "${REPO}" status --porcelain --untracked-files=no)" ]] \
    || fail "tracked_worktree_must_be_clean_before_v12_training"
fi
...
if [[ "${DRY_RUN}" == "0" ]]; then
  for critical_file in "${critical_tracked_files[@]}"; do
    git -C "${REPO}" ls-files --error-unmatch -- "${critical_file}" >/dev/null 2>&1 \
      || fail "critical_v12_source_must_be_tracked path=${critical_file}"
  done
fi
...
contract_args=(
  -m experiments.rethinking_rwkv_ms_gemma.scene_memory_v12_launch_contract
  --target-step 2
  --data-root "${DATA_ROOT}"
...
if ! contract_values="$("${VALIDATION_PYTHON_BIN}" "${contract_args[@]}")"; then
  fail "v12_launch_contract_validation_failed"
fi
...
[[ "${VALIDATED_TARGET_STEP}" == "2" ]] || fail "validated_target_step_differs"
[[ "${SOURCE_STEP}" == "0" && "${RESUME_SCHEDULE_CURSOR}" == "0" ]] \
  || fail "v12_must_begin_fresh_at_zero"
...
train_args=(
  ...
  --warm-start-mode scene_memory_v12_v8_checkpoint56_adapter_only
  --resume-mode exact
  ...
  --scene-state-generation-objective-version scene_state_generation_ce_symmetric_semantic_boundary_margin_v12
...
printf 'Validated V12 fresh two-cycle launch; first pair=%s,%s\n' \
...
printf 'Hard32, full170, test, validation, other benchmarks, and resume are forbidden.\n'

if [[ "${DRY_RUN}" == "1" ]]; then
  printf 'Validated V12 training command (not started):\n'
...
payload = {
    ...
    "warmup_steps": 0,
    "save_total_limit": 2,
    "two_cycle_pairs": [list(pair) for pair in launch.TWO_CYCLE_PAIRS],
    "two_cycle_pairs_sha256": launch.TWO_CYCLE_PAIRS_SHA256,
...

{
  ...
  printf 'Starting attached V12 two-cycle training. Command: '
  printf '%q ' "${train_command[@]}"
  printf '\n'
} | tee "${LOG_FILE}"
...
set -e
(( pipeline_status[0] == 0 )) \
  || fail "v12_training_failed exit_code=${pipeline_status[0]} log=${LOG_FILE}"
(( pipeline_status[1] == 0 )) \
  || fail "v12_training_log_failed exit_code=${pipeline_status[1]}"
...
output_raw, launch_raw, log_raw, summary_raw, checkpoint1_raw, checkpoint2_raw = sys.argv[1:]
checkpoints = [Path(checkpoint1_raw).resolve(), Path(checkpoint2_raw).resolve()]
...
payload = {
    ...
    "status": "completed",
    "optimizer_step": 2,
    "consumed_pair_presentations": 14,
    "launch_receipt": launch_validation["artifact"],
    "launch_receipt_sha256": launch_validation["receipt_sha256"],
    "log": launch.artifact_binding(Path(log_raw), description="v12_completion_log"),
        ...
        checkpoint.name: {
            ...
            "checkpoint_artifacts": {
                name: launch.artifact_binding(
                    checkpoint / name,
                    description=f"v12_completion_{checkpoint.name}_{name}",
                )
            ...
            "rng_state_artifacts": {
                path.name: launch.artifact_binding(
                    path,
                    description=f"v12_completion_{checkpoint.name}_{path.name}",
                )
...
printf 'Completed attached V12 optimizer checkpoints 1 and 2: %s\n' "${OUTPUT_DIR}"
printf 'Gate provenance: --launch-receipt %s --completion-receipt %s\n' \
  "${LAUNCH_RECEIPT}" "${COMPLETION_RECEIPT}"
printf 'Gate both V12 checkpoints on Value14 only; never resume this run.\n'
