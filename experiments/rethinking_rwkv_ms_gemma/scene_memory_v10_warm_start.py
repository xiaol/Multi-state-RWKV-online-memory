#!/usr/bin/env python3
"""V10 identity wrapper for the pinned V8 checkpoint-56 adapter warm start.

V10 intentionally reuses the byte-locked V9 source adapter.  This module keeps
that source validation unchanged while giving the target run a distinct mode
and receipt schema, so a V9 target checkpoint cannot be mistaken for V10.
"""
...
WARM_START_MODE = "scene_memory_v10_v8_checkpoint56_adapter_only"
RECEIPT_SCHEMA = "rwkv_ms_scene_memory_v10_adapter_warm_start_receipt.v1"
...
def load_v10_warm_start_lock(
    lock_path: str | Path = DEFAULT_LOCK_PATH,
) -> dict[str, Any]:
    """Validate and return the shared pinned V8 artifact lock."""

    return v9.load_v9_warm_start_lock(lock_path)


def prepare_v10_v8_checkpoint56_warm_start(
    source_checkpoint: str | Path | None,
    *,
    lock_path: str | Path = DEFAULT_LOCK_PATH,
) -> V10WarmStartContext:
    """Validate the V8 source, then bind it to the V10 target identity."""

    source = v9.prepare_v9_v8_checkpoint56_warm_start(
        source_checkpoint,
        lock_path=lock_path,
    )
    return V10WarmStartContext(
...
def validate_v10_fresh_start_contract(
    contract: V10FreshStartContract,
) -> dict[str, Any]:
    if contract.resume_from_checkpoint is not None:
        raise ValueError("V10 adapter warm start cannot import checkpoint resume state")
    if isinstance(contract.initial_global_step, bool) or contract.initial_global_step != 0:
        raise ValueError("V10 adapter warm start must begin at global step 0")
    imported = {
    ...
    if forbidden:
        raise ValueError(
            "V10 adapter warm start forbids preloaded training state: "
            + ", ".join(forbidden)
        )
    if contract.optim not in ALLOWED_FRESH_ADAMW_IMPLEMENTATIONS:
        raise ValueError(
            "V10 adapter warm start requires a fresh AdamW optimizer created after load"
        )
    return {
...
def apply_v10_v8_checkpoint56_adapter_only_warm_start(
    model: nn.Module,
    context: V10WarmStartContext,
    *,
    fresh_start: V10FreshStartContract,
) -> dict[str, Any]:
    """Load only the pinned V8 adapter and emit V10-native lineage evidence."""

    fresh_start_receipt = validate_v10_fresh_start_contract(fresh_start)
    ...
    if unequal:
        raise ValueError(
            "V10 warm-start adapter tensors are not bit-equal after loading: "
            + ", ".join(unequal[:8])
        )
...
def write_v10_warm_start_receipt(
    ...
    if recorded_sha256 != v9.canonical_sha256(payload):
        raise ValueError("V10 warm-start receipt self-hash differs")
...
__all__ = (
    ...
    "validate_v10_fresh_start_contract",
    "write_v10_warm_start_receipt",
)
