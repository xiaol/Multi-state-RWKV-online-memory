#!/usr/bin/env python3
"""Continue query-value recurrent routing with weighted narrative paraphrases."""
...
from experiments.rethinking_rwkv_ms_gemma import run_natural_memory_native_recurrent_routed_query_value_gain_half as predecessor_runner  # noqa: E402


MODE = "recurrent_routed_query_value"
PROTOCOL = SCRIPT_DIR / "natural_memory_native_recurrent_routed_query_value_narrative_repair_protocol_v1.json"
PROTOCOL_PAYLOAD_SHA256 = "de5a0d28a42d95e215ae6460c8a0e9aecc00b31a36dbbf695529268a96ca13c1"
PREDECESSOR_ROOT = SCRIPT_DIR / "local_artifacts/recurrent_routed_query_value_gain_half_train20_v1"
PREDECESSOR_RESULT_RECEIPT = "3336b3ee3df5bb18f7cf8d41dd97c79b0b513914312b3313d1661a14eec439ae"
PREDECESSOR_ADAPTER_WEIGHTS_SHA256 = "1f52598564a7d9a2b0524dadfa172a2145b36e7ebbb8508bf0edb49b89284590"
PREDECESSOR_ADAPTER_CONFIG_SHA256 = "ad0da606f60ce44585094d8b3b8ebb1eca56b0d0a885fa01801d643d3800f724"
PREDECESSOR_DEVELOPMENT_ROOT = SCRIPT_DIR / "local_artifacts/recurrent_routed_query_value_gain_half_development_v2"
PREDECESSOR_DEVELOPMENT_RECEIPT = "7f94b255d0280eb53a48e0767b0eccae4b5197ad9be54b69b4099e833b7a1981"
...
def validate_lineage() -> tuple[Mapping[str, Any], Mapping[str, Any]]:
    ...
    predecessor_development = common.validate_signed_json(
        PREDECESSOR_DEVELOPMENT_ROOT / "result.json",
        PREDECESSOR_DEVELOPMENT_RECEIPT,
    )
    manifest = load_v2_manifest()
    if (
        predecessor.get("status") != "query_value_training_complete_development_v2_evaluation_authorized"
        or predecessor.get("passed") is not True
        or predecessor.get("final_rows_opened") is not False
        or predecessor_development.get("status") != "development_failed_final_evaluation_blocked"
        or predecessor_development.get("passed") is not False
        or predecessor_development.get("final_rows_opened") is not False
        ...
        raise ValueError("Narrative-repair predecessor lineage differs")
...
def stage13_schedule(
    ...
    previous_schedule, previous_payload, _ = predecessor_runner.stage12_schedule(rows_by_task)
    prior_targets = {
        row.target.row_sha256
        for row in full_schedule
        if row.step < 97
    }
    prior_targets.update(row.target.row_sha256 for row in previous_schedule)
    development_ordinals = load_v2_manifest()["development_source_ordinals"]
    development = {task: set(ordinals) for task, ordinals in development_ordinals.items()}
    ...
    for task in common.TASKS:
        ...
        candidates.sort(
            key=lambda row: hashlib.sha256(
                f"query-value-stage13:{task}:{row.row_sha256}".encode("utf-8")
            ).hexdigest()
        )
        available[task] = candidates
    target_counts = {"attribution": 4, "narrative": 32, "scene": 4}
    selected_targets = {
        task: tuple(available[task][: target_counts[task]])
        for task in common.TASKS
    }
    if any(len(selected_targets[task]) != target_counts[task] for task in common.TASKS):
        raise RuntimeError("Narrative-repair untouched schedule differs")
    ...
    for task in common.TASKS:
        for target in selected_targets[task]:
            ...
            if not candidates:
                raise RuntimeError(f"Narrative-repair target has no donor: {target}")
    ...
    while any(remaining.values()):
        task = max(
            ...
            key=lambda value: (
                ...
                hashlib.sha256(
                    f"query-value-stage13-task:{value}:{len(targets)}".encode()
                ).hexdigest(),
            ),
        )
        targets.append(remaining[task].pop(0))
    if len(targets) != TRAIN_UPDATES * 2:
        raise RuntimeError("Narrative-repair target count differs")
    ...
    for step in range(1, TRAIN_UPDATES + 1):
        ...
        for target in step_targets:
            ...
            variants = (0, 1, 0, 1) if target.task == "narrative" else (0, 1, 2, 3)
            for variant in variants:
                step_rows.append((target, donor, variant))
        step_rows.sort(
            key=lambda item: hashlib.sha256(
                f"query-value-stage13-step:{step}:{item[0].row_sha256}:{item[2]}".encode()
            ).hexdigest()
        )
        step_payload = []
    ...
    return tuple(selected), selected_payload, common.canonical_sha256(
        {
            "prior_96_steps": full_payload,
            "stage12_steps": previous_payload,
            "stage13_steps": selected_payload,
        }
    )


def load_predecessor_adapter(model: torch.nn.Module, input_dir: Path) -> Any:
    state = torch.load(
...
def configure() -> None:
    ...
    stage2.PREFLIGHT_STATUS = "query_value_narrative_repair_preflight_passed"
    stage2.TRAINING_STATUS = "query_value_narrative_repair_training_complete_development_v2_evaluation_authorized"
    stage2.FAILURE_STATUS = "query_value_narrative_repair_training_failed_development_v2_evaluation_blocked"
    ...
    stage2.stage2_schedule = stage13_schedule
    stage2.load_delta_mem_adapter = load_predecessor_adapter
    stage2.stage1.train = predecessor_runner.train_with_always_on_donor
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if PROTOCOL_PAYLOAD_SHA256 == "PLACEHOLDER":
        raise ValueError("Narrative-repair protocol receipt is not installed")
    args = parse_args(argv)
    configure()
    context = distributed.initialize_distributed_training(args.device)
    if context is None:
        raise ValueError("Narrative-repair post-training requires four-rank torchrun")
    try:
        result = stage2.run(
            context=context,
