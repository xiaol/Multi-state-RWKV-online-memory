#!/usr/bin/env python3
"""Evaluate one shard of the trained addressed-value RWKV bottleneck."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_rwkv_addressed_value_causal_train as training,
)


SCHEMA = "rwkv_ms_natural_memory_native_addressed_value_eval.v1"
INPUT_SCHEMA = "rwkv_ms_natural_memory_native_addressed_value_eval_input.v1"
...
def zero_state(state: Mapping[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    return {key: torch.zeros_like(value) for key, value in state.items()}


def stack_condition_states(
    states: Mapping[str, Mapping[str, torch.Tensor]],
) -> dict[str, torch.Tensor]:
    if tuple(states) != CONDITIONS:
        raise ValueError("Addressed-value condition state order differs")
    key_sets = {frozenset(state) for state in states.values()}
    if len(key_sets) != 1:
        raise ValueError("Addressed-value condition state keys differ")
    keys = next(iter(key_sets))
    return {
        key: torch.cat([states[condition][key] for condition in CONDITIONS], dim=0)
        for key in keys
    }


def batched_generate_from_states(
    model,
    tokenizer,
    row: Mapping[str, Any],
    states: Mapping[str, Mapping[str, torch.Tensor]],
    *,
    ...
    base_eval.reset_delta_state(model)
    base_eval.load_delta_mem_online_state(model, stack_condition_states(states))
    base_eval.set_delta_write_enabled(model, False)
    encoded = base_eval.causal.encode_prompt(
        tokenizer,
        row["messages"],
        generation=True,
    )
    input_ids = encoded.input_ids.to(device).expand(len(CONDITIONS), -1).clone()
    attention_mask = (
        encoded.attention_mask.to(device).expand(len(CONDITIONS), -1).clone()
    )
    ...
    with torch.inference_mode():
        output_ids = model.generate(
            ...
            generation_config=base_eval.causal.generation_config(model, tokenizer),
        )
    ...
    for condition, raw in zip(CONDITIONS, raw_generations):
        ...
        results[condition] = {
            ...
            "hit_max_new_tokens": (
                int(generated_ids.size(1)) >= base_eval.causal.MAX_NEW_TOKENS
            ),
            "elapsed_seconds": elapsed,
            "peak_cuda_memory_bytes": peak_memory,
            "generation_batch_conditions": len(CONDITIONS),
        }
    base_eval.reset_delta_state(model)
    base_eval.set_delta_write_enabled(model, True)
    return results
...
def validate_train_result(
    ...
    *,
    ...
    if not isinstance(receipt, Mapping):
        raise ValueError("Addressed-value training result receipt is missing")
    ...
    if (
        ...
        raise ValueError("Addressed-value training did not authorize evaluation")
    expected_files = result.get("adapter_files")
    if not isinstance(expected_files, Mapping):
        raise ValueError("Addressed-value adapter manifest is missing")
    for filename, metadata in expected_files.items():
        ...
        if not path.is_file() or sha256_file(path) != metadata.get("sha256"):
            raise ValueError(f"Addressed-value adapter file differs: {path}")
    if result.get("code_bindings", {}).get("runner_sha256") != sha256_file(
        Path(training.__file__)
    ):
        raise ValueError("Addressed-value training runner binding differs")
    return result
...
def generate_row_conditions(
    ...
    *,
    ...
    if base_eval.tensor_digest(donor_projected) == projected_digest:
        raise RuntimeError("Addressed-value donor projected carrier is unexpectedly equal")
    conditions = {
        ...
        "empty_memory": zero_state(correct_state),
    }
    generated_conditions = batched_generate_from_states(
        model,
        tokenizer,
        row,
        conditions,
    ...
    for condition in CONDITIONS:
        ...
        if not carrier_fixed:
            raise RuntimeError("Addressed-value projected carrier changed")
        records.append(
            {
                "schema": SCHEMA,
                "protocol_payload_sha256": PROTOCOL_PAYLOAD_SHA256,
...
def run(args: argparse.Namespace) -> int:
    ...
    if not 0 <= args.shard_index < WORLD_SIZE:
        raise ValueError("Addressed-value shard index is outside the four-way split")
    if (
        args.partitions_per_shard < 1
        or not 0 <= args.partition_index < args.partitions_per_shard
    ):
        raise ValueError("Addressed-value partition is outside the shard split")
    ...
    if sha256_file(dataset_file) != DATASET_SHA256:
        raise ValueError("Addressed-value evaluation dataset hash differs")
    ...
    if len(module_names) != training.preflight.EXPECTED_LAYERS:
        raise ValueError(f"Expected 42 addressed-value layers, found {len(module_names)}")
    ...
    for condition, records in existing.items():
        if not set(records) <= set(expected_rows):
            raise ValueError(f"Unexpected addressed-value resumed rows: {condition}")
        for source_index, result in records.items():
            ...
            if any(result.get(key) != value for key, value in required.items()):
                raise ValueError(f"Addressed-value resumed record differs: {source_index}")
    if all(len(existing[condition]) == len(shard_rows) for condition in CONDITIONS):
        print(f"ADDRESSED_VALUE_EVAL_SHARD_COMPLETE shard={args.shard_index}", flush=True)
        return 0
    started = time.time()
    for ordinal, row in enumerate(shard_rows, start=1):
        ...
        print(
            f"ADDRESSED_VALUE_EVAL_PROGRESS shard={args.shard_index} "
    ...
    print(
        f"ADDRESSED_VALUE_EVAL_SHARD_COMPLETE shard={args.shard_index} "
        f"partition={args.partition_index}/{args.partitions_per_shard}",
        flush=True,
    )
    return 0
