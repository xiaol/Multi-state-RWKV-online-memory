#!/usr/bin/env python3
"""Train chunk-aligned addressed RWKV values against causal controls."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_rwkv_addressed_value_causal_train as addressed_train,
)
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
...
SCHEMA = "rwkv_ms_natural_memory_native_chunk_addressed_value_causal_train.v1"
STEP_SCHEMA = (
    "rwkv_ms_natural_memory_native_chunk_addressed_value_causal_train_step.v1"
)
INPUT_SCHEMA = (
    "rwkv_ms_natural_memory_native_chunk_addressed_value_causal_train_input.v1"
)
PROTOCOL = (
    SCRIPT_DIR
    / "natural_memory_native_rwkv_chunk_addressed_value_causal_train_protocol_v1.json"
)
PROTOCOL_PAYLOAD_SHA256 = (
    "c544e1ba633127f8129356ea248ae4d4e8014cb39cf8492685de06f3d71ffb37"
)
CALIBRATION_RESULT = (
    SCRIPT_DIR
    / "local_artifacts/"
    "natural_memory_native_rwkv_chunk_addressed_value_calibration_v1/result.json"
)
CALIBRATION_RESULT_FILE_SHA256 = (
    "51750f9884619baa1a430cbcf50927d2221ab3a0c6ee64660db1f4506a16880a"
)
CALIBRATION_RESULT_RECEIPT = (
    "0ccb35783c91050896f2eb624eafbec12c9e9fef83d539e19fbdaafdd2822ca7"
)
...
def write_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Chunk-addressed causal output must be fresh: {path}")
...
def validate_protocol() -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, Mapping):
        raise ValueError("Chunk-addressed causal protocol receipt is missing")
    unsigned = dict(protocol)
    unsigned.pop("receipt")
    digest = canonical_sha256(unsigned)
    if digest != PROTOCOL_PAYLOAD_SHA256 or receipt.get("payload_sha256") != digest:
        raise ValueError("Chunk-addressed causal protocol payload differs")
    ...
    required_authorization = {
        ...
        "calibration_result_file": (
            "local_artifacts/"
            "natural_memory_native_rwkv_chunk_addressed_value_calibration_v1/"
            "result.json"
        ),
    ...
    if authorization != required_authorization:
        raise ValueError("Chunk-addressed causal authorization differs")
    ...
    if any(training.get(key) != value for key, value in required_training.items()):
        raise ValueError("Chunk-addressed causal training contract differs")
    if protocol.get("protected_splits_opened_by_this_protocol") != []:
        raise ValueError("Chunk-addressed causal training may not open protected data")
    return protocol


def validate_calibration_result() -> Mapping[str, Any]:
    if sha256_file(CALIBRATION_RESULT) != CALIBRATION_RESULT_FILE_SHA256:
        raise ValueError("Chunk-addressed calibration result file hash differs")
    result = json.loads(CALIBRATION_RESULT.read_text(encoding="utf-8"))
    receipt = result.get("receipt")
    if not isinstance(receipt, Mapping):
        raise ValueError("Chunk-addressed calibration result receipt is missing")
    ...
    if (
        ...
        raise ValueError("Chunk-addressed calibration did not authorize training")
    return result
...
def load_model(
    ...
    *,
    ...
    wrappers_valid = all(
        ...
        and module.rwkv_ms_hybrid_mode == "chunk_addressed_value"
    ...
    audit = {
        ...
        "all_wrappers_chunk_addressed_value": wrappers_valid,
    ...
    if (
        ...
        raise RuntimeError(f"Chunk-addressed causal attachment failed: {audit!r}")
...
def train(
    ...
    *,
    ...
    previous_step_schema = addressed_train.STEP_SCHEMA
    previous_protocol_hash = addressed_train.PROTOCOL_PAYLOAD_SHA256
    addressed_train.STEP_SCHEMA = STEP_SCHEMA
    addressed_train.PROTOCOL_PAYLOAD_SHA256 = PROTOCOL_PAYLOAD_SHA256
    try:
        return addressed_train.train(
            model,
            rows,
            schedule,
            updates=updates,
        ...
        addressed_train.STEP_SCHEMA = previous_step_schema
        addressed_train.PROTOCOL_PAYLOAD_SHA256 = previous_protocol_hash
...
def run(
    *,
    ...
    if context.world_size != WORLD_SIZE:
        raise ValueError("Chunk-addressed causal training requires exactly four ranks")
    if updates not in (PREFLIGHT_UPDATES, TRAIN_UPDATES):
        raise ValueError("Chunk-addressed causal updates must be 1 or 8")
    ...
    if sha256_file(base_model / "config.json") != preflight.EXPECTED_BASE_CONFIG_SHA256:
        raise ValueError("Chunk-addressed causal pinned base config differs")
    ...
    if context.is_primary and resolved_output.exists():
        freshness_error = ValueError(
            f"Chunk-addressed causal output must be fresh: {resolved_output}"
        )
    distributed.phase_consensus(
        context,
        phase="chunk-addressed-causal-output-freshness",
    ...
    distributed.phase_consensus(
        context,
        phase="chunk-addressed-causal-output-creation",
    ...
    if (
        ...
        raise RuntimeError("Chunk-addressed causal schedule binding differs")
    input_binding = {
        "schema": INPUT_SCHEMA,
    ...
    distributed.require_consensus(
        ...
        description="chunk-addressed causal input binding",
    )
    ...
    distributed.phase_consensus(
        context,
        phase="chunk-addressed-causal-input-binding-save",
        error=binding_error,
    )
    training = train(
        model,
    ...
    distributed.phase_consensus(
        context,
        phase="chunk-addressed-causal-result-save",
        error=save_error,
    )
    del model, rows
    gc.collect()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if context is None:
        raise ValueError("Chunk-addressed causal training requires four-rank torchrun")
    try:
        result = run(
            context=context,
