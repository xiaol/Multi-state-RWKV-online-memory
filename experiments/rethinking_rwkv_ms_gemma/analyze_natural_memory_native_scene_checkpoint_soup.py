#!/usr/bin/env python3
"""Cross-fit and sign the locked native scene checkpoint-soup study."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_scene_checkpoint_soup as runner,
)
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
...
SCHEMA = "rwkv_ms_natural_memory_native_scene_checkpoint_soup_result.v1"
FOLD_SALT = "rwkv-ms-native-scene-checkpoint-soup-v1:"
FOLDS = 5
FOLD_ASSIGNMENT_PAYLOAD_SHA256 = "c14e085f7c7a814cccfa5eaa0afa323f097cef02ce62e8212bf3f447b287cd65"
FOLD_COUNTS = {0: 41, 1: 45, 2: 54, 3: 39, 4: 41}
...
GATE_THRESHOLDS = {
    ...
    "learned_soup_selected_folds": 4,
    "modal_learned_soup_selected_folds": 3,
    "modal_soup_minus_checkpoint_16_micro_f1": 0.005,
}
...
def validate_folds(rows: Sequence[Mapping[str, Any]]) -> Mapping[int, tuple[int, ...]]:
    ...
    if canonical_sha256(payload) != FOLD_ASSIGNMENT_PAYLOAD_SHA256:
        raise ValueError("Checkpoint-soup fold assignment differs")
    indices = {
        fold: tuple(
    ...
    if {fold: len(value) for fold, value in indices.items()} != FOLD_COUNTS:
        raise ValueError("Checkpoint-soup fold counts differ")
    return indices


def read_soup_outputs(
    root: Path,
    *,
    ...
    for shard_index in range(runner.WORLD_SIZE):
        ...
        if any(binding.get(key) != value for key, value in required_binding.items()):
            raise ValueError(f"Checkpoint-soup input binding differs: {binding_path}")
        bindings.append(
        ...
        for candidate_id, manifest in manifest_by_id.items():
            ...
            for record in records:
                ...
                if (
                    ...
                    raise ValueError(f"Checkpoint-soup output differs: {candidate_id}:{source_index}")
                seen.add(source_index)
                outputs[candidate_id][source_index] = record
            if seen != shard_expected:
                raise ValueError(f"Incomplete checkpoint-soup shard: {candidate_id}:{shard_index}")
    for candidate_id, records in outputs.items():
        if set(records) != set(expected):
            raise ValueError(f"Incomplete checkpoint-soup candidate: {candidate_id}")
...
def validate_progression_result(root: Path) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ValueError("Checkpoint-soup progression result binding differs")
    return result
...
def analyze(
    *,
    ...
    soup_outputs, materialization, manifests, bindings, artifacts = read_soup_outputs(
        input_root,
    ...
    soup_predictions = {
        ...
        for candidate_id, records in soup_outputs.items()
    }
    ...
    predictions = {**soup_predictions, CHECKPOINT_ID: checkpoint_predictions}
    aggregate_metrics = {
    ...
    if set(oof_predictions) != set(indices):
        raise ValueError("Checkpoint-soup out-of-fold coverage differs")
    ...
    deltas = {
        ...
        "modal_soup_minus_checkpoint_16_micro_f1": (
            None
    ...
    gates = {
        ...
        "learned_soup_selected_in_at_least_4_folds": len(learned_selected)
        >= GATE_THRESHOLDS["learned_soup_selected_folds"],
        "modal_learned_soup_selected_in_at_least_3_folds": modal_count
        >= GATE_THRESHOLDS["modal_learned_soup_selected_folds"],
        "modal_soup_minus_checkpoint_16_micro_f1_at_least_0.005": (
            deltas["modal_soup_minus_checkpoint_16_micro_f1"] is not None
            and deltas["modal_soup_minus_checkpoint_16_micro_f1"]
            >= GATE_THRESHOLDS["modal_soup_minus_checkpoint_16_micro_f1"]
        ),
    }
    ...
    result: dict[str, Any] = {
        ...
        "candidate_ids": list(soup_predictions),
        "aggregate_metrics": aggregate_metrics,
        "v9_metrics": v9_metrics,
        "fold_results": fold_results,
        "fold_selected_candidate_ids": selected_ids,
        "learned_soup_selected_folds": len(learned_selected),
        "learned_soup_selection_counts": dict(sorted(learned_counts.items())),
    ...
    result["receipt"] = {
        "algorithm": "sha256",
        "payload_scope": "canonical_checkpoint_soup_result_without_receipt",
        "payload_sha256": canonical_sha256(result),
    }
    if output.exists():
        raise ValueError(f"Checkpoint-soup result must be fresh: {output}")
