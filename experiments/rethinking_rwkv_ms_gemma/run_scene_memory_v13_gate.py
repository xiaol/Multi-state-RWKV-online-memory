#!/usr/bin/env python3
"""Run the exact-Value14-only Scene Memory V13 candidate gate."""
...
GATE_CONTRACT = "scene_memory_v13_exact_value14_candidate_gate"
GATE_RECORD_SCHEMA = "rwkv_ms_scene_memory_v13_value14_gate_record.v1"
GATE_SUMMARY_SCHEMA = "rwkv_ms_scene_memory_v13_value14_gate_summary.v1"
GATE_MANIFEST_SCHEMA = "rwkv_ms_scene_memory_v13_value14_gate_manifest.v1"
GATE_RECEIPT_SCHEMA = "rwkv_ms_scene_memory_v13_value14_gate_receipt.v1"
CANDIDATE_DESIGNATION_KIND = "scene_memory_v13_value14_candidate_receipt"
...
V13_OBJECTIVE = {
    **v10.V10_OBJECTIVE,
    ...
    "progression_basis": "four_cycle_value14_dense_boundary_fit_v1",
    "dense_semantic_mode": launch.DENSE_SEMANTIC_MODE,
    "dense_decision_scope": launch.DENSE_DECISION_SCOPE,
    "dense_top1_retention_margin": launch.SEMANTIC_MARGIN,
...
class V13EvaluationContractError(ValueError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise V13EvaluationContractError(message)
...
def _run_path(path: Path | str, *, description: str, ssd_root: Path) -> Path:
    try:
        return launch.require_v13_run_path(
            path,
            description=description,
            ssd_root=ssd_root,
        )
    except Exception as exc:
        raise V13EvaluationContractError(
            f"V13 {description} must stay under the locked V13 run root: {exc}"
        ) from exc


def _gate_path(path: Path | str, *, description: str, ssd_root: Path) -> Path:
    try:
        return launch.require_v13_gate_path(
            path,
            description=description,
            ssd_root=ssd_root,
        )
        ...
        raise V13EvaluationContractError(
            f"V13 {description} must stay under the locked V13 gates root: {exc}"
        ) from exc
    ...
    *,
    ...
    try:
        _require(
            ...
            "V13 pinned base model override is forbidden",
        )
        ...
        raise V13EvaluationContractError(
            "V13 base model must equal the model and prompt artifacts in the "
...
def _regular_file(
    ...
    *,
    ...
    _require(resolved.is_file(), f"V13 {description} is missing: {resolved}")
    return resolved
...
def _load_json(path: Path | str, *, description: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise V13EvaluationContractError(f"V13 {description} is invalid JSON") from exc
    _require(isinstance(payload, dict), f"V13 {description} must be an object")
    return payload
...
def _read_jsonl(path: Path, *, description: str) -> list[dict[str, Any]]:
    ...
    for row_number, line in enumerate(
        ...
        _require(bool(line.strip()), f"V13 {description} contains a blank row")
        try:
            payload = json.loads(line)
        except json.JSONDecodeError as exc:
            raise V13EvaluationContractError(
                f"V13 {description} row {row_number} is invalid JSON"
            ) from exc
        _require(isinstance(payload, dict), f"V13 {description} row must be an object")
...
def _validate_self_hash(payload: Mapping[str, Any], *, field: str) -> str:
    ...
    _require(isinstance(recorded, str), f"V13 {field} is missing")
    _require(
...
def validate_v13_train_inputs(
    *,
    ...
    try:
        ...
        raise V13EvaluationContractError(f"V13 input contract failed: {exc}") from exc
    _require(
        ...
        "V13 four-cycle identity differs",
    )
    result = dict(base)
    result.update(
        {
...
def _validate_v13_objective_protocol(
    ...
    *,
    ...
    try:
        ...
        raise V13EvaluationContractError(f"V13 objective protocol differs: {exc}") from exc
    ...
    _require(
        ...
        "V13 objective source identity differs",
    )


def validate_v13_checkpoint(
    memory_dir: Path | str,
    *,
    input_contract: Mapping[str, Any],
    ...
    try:
        ...
        raise V13EvaluationContractError(f"V13 checkpoint contract failed: {exc}") from exc
    resolved = Path(str(lineage["checkpoint"]))
    checkpoint_step = int(lineage["checkpoint_step"])
    protocol = _load_json(
        resolved / "training_protocol.json",
        description="V13 training protocol",
    )
    _validate_v13_objective_protocol(protocol, input_contract=input_contract)
    pairing = _load_json(
        resolved / "scene_state_identity_pairing_manifest.json",
        description="V13 checkpoint pairing manifest",
    )
    try:
        ...
        raise V13EvaluationContractError(f"V13 pairing contract failed: {exc}") from exc
    ...
    _require(
        ...
        "V13 checkpoint architecture differs",
    )
    artifacts = {
        name.removesuffix(".json").removesuffix(".pt").removesuffix(".pth"): (
            _artifact_binding(resolved / name, description=f"V13 checkpoint {name}")
        )
        for name in launch.REQUIRED_CHECKPOINT_ARTIFACTS
    }
    rng = [
        _artifact_binding(path, description=f"V13 checkpoint RNG {path.name}")
    ...
    checkpoint_contract = {
        ...
        "lineage_artifact": _artifact_binding(
            lineage_path,
            description="V13 checkpoint lineage",
        ),
        "architecture": architecture,
        "objective": dict(V13_OBJECTIVE),
    ...
    try:
        ...
        raise V13EvaluationContractError(
            f"V13 training provenance failed: {exc}"
        ) from exc
...
def build_v13_gate(
    *,
    ...
    _require(
        ...
        "V13 gate accepts only a locked checkpoint from 1 through 4",
    )
    _require(
        set(records_by_condition) == set(CONDITIONS),
        "V13 gate conditions differ",
    )
    ...
    for condition in CONDITIONS:
        ...
        _require(
            ...
            f"V13 gate requires exactly 14 {condition} rows",
        )
        ...
        for expected_ordinal, record in zip(VALUE14_ORDINALS, records, strict=True):
            ...
            _require(
                ...
                "V13 gate rows must be exactly Value14 in canonical order",
            )
            _require(record.get("condition") == condition, "V13 gate condition differs")
    ...
    _require(
        ...
        "V13 Value14 donor mapping differs",
    )
    scores = {
...
def validate_resume_records(
    ...
    *,
    ...
    _require(condition in CONDITIONS, "V13 resume condition differs")
    _require(
        len(records) <= len(VALUE14_ORDINALS),
        "V13 resume records exceed Value14",
    )
    validated: dict[int, dict[str, Any]] = {}
    for position, record in enumerate(records):
        current = dict(record)
        _require(current.get("schema") == GATE_RECORD_SCHEMA, "V13 record schema differs")
        _validate_self_hash(current, field="record_sha256")
        ordinal = VALUE14_ORDINALS[position]
        _require(current.get("fingerprint") == fingerprint, "V13 record fingerprint differs")
        _require(current.get("condition") == condition, "V13 record condition differs")
        _require(current.get("split") == "train", "V13 record split differs")
        _require(
            current.get("train_row_ordinal") == ordinal,
            "V13 records are not a canonical Value14 prefix",
        )
        sample = rows[ordinal]
        expected_donor = donor_by_ordinal[ordinal]
        _require(current.get("source_index") == sample["source_index"], "V13 source differs")
        _require(current.get("row_sha256") == sample["row_sha256"], "V13 row hash differs")
        _require(current.get("gold") == sample["gold"], "V13 record gold differs")
        _require(
            current.get("donor_train_row_ordinal") == expected_donor
            and current.get("donor_source_index")
            == rows[expected_donor]["source_index"],
            "V13 donor identity differs",
        )
        raw_generation = current.get("raw_generation")
        _require(isinstance(raw_generation, str), "V13 raw generation differs")
        _require(
            v10.v9.v8.extract_json(raw_generation) == current.get("parsed_json"),
            "V13 parsed JSON differs",
        )
...
def evaluator_code_binding() -> dict[str, Any]:
    paths = {
        "v13_gate": Path(__file__).resolve(),
        ...
        "v13_launch_contract": Path(launch.__file__).resolve(),
        "v13_warm_start": Path(v13_warm.__file__).resolve(),
    ...
    return {
        name: _artifact_binding(path, description=f"V13 evaluator code {name}")
...
def build_evaluation_fingerprint_payload(
    *,
    ...
    _require(
        ...
        "V13 fingerprint checkpoint layer identity differs",
    )
    _require(
        isinstance(checkpoint.get("training_provenance"), Mapping),
        "V13 fingerprint training provenance is missing",
    )
    return {
        "schema_version": 1,
        "contract": GATE_CONTRACT,
...
def validate_existing_manifest(
    ...
    *,
    ...
    _require(
        ...
        "V13 existing manifest fields differ",
    )
    if require_postload:
        _require(set(manifest) == postload_keys, "V13 completed manifest lacks runtime identity")
    _require(manifest.get("schema") == GATE_MANIFEST_SCHEMA, "V13 manifest schema differs")
    _require(
        isinstance(manifest.get("created_at"), str) and bool(manifest["created_at"]),
        "V13 manifest creation time differs",
    )
    expected_payload = dict(expected_fingerprint_payload)
    _require(
        manifest.get("fingerprint_payload") == expected_payload,
        "V13 manifest fingerprint payload differs",
    )
    _require(
        manifest.get("fingerprint") == expected_fingerprint
        and expected_fingerprint
        == v10.v9.fingerprint_payload_sha256(expected_payload),
        "V13 manifest fingerprint differs",
    )
    _require(
        manifest.get("hard32_access") == HARD32_ACCESS_POLICY,
        "V13 manifest Hard32 identity differs",
    )
    if "runtime_prefixes" in manifest:
        _require(
            ...
            "V13 manifest runtime identity differs",
        )
...
def bind_or_validate_manifest_runtime(
    ...
    *,
    ...
    _require(has_prefixes == has_profile, "V13 manifest runtime fields are incomplete")
    if has_prefixes:
        _require(bound["runtime_prefixes"] == runtime_prefixes, "V13 runtime prefixes differ")
        _require(
            bound["runtime_fusion_profile"] == runtime_fusion_profile,
            "V13 runtime fusion profile differs",
        )
        return bound
...
def build_gate_receipt(
    *,
    ...
    output_dir = _gate_path(
        output_dir,
        description="V13 gate output directory",
        ssd_root=ssd_root,
    )
    outputs = {
        "manifest": _artifact_binding(
            output_dir / "manifest.json",
            description="V13 gate manifest",
        ),
        "summary": _artifact_binding(
            output_dir / "summary.json",
            description="V13 gate summary",
        ),
        "conditions": {
            condition: _artifact_binding(
                output_dir / f"{condition}.jsonl",
                description=f"V13 {condition}",
            )
            for condition in CONDITIONS
        },
    }
...
def _verify_artifact_binding(
    ...
    *,
    ...
    _require(isinstance(binding, Mapping), f"V13 {description} binding is missing")
    raw_path = binding.get("path")
    _require(
        isinstance(raw_path, str) and bool(raw_path),
        f"V13 {description} path differs",
    )
    expected = _gate_path(
        raw_path,
        description=description,
    ...
    try:
        ...
        raise V13EvaluationContractError(f"V13 {description} differs: {exc}") from exc
    _require(actual == expected, f"V13 {description} resolved path differs")
    return actual
    ...
    *,
    ...
    if isinstance(receipt, Mapping):
        ...
        receipt_path = _regular_file(
            receipt,
            description="V13 gate receipt",
            ssd_root=ssd_root,
        )
        payload = _load_json(receipt_path, description="V13 gate receipt")
    _require(payload.get("schema") == GATE_RECEIPT_SCHEMA, "V13 receipt schema differs")
    _validate_self_hash(payload, field="receipt_sha256")
    _require(payload.get("contract") == GATE_CONTRACT, "V13 receipt contract differs")
    _require(payload.get("objective") == V13_OBJECTIVE, "V13 receipt objective differs")
    _require(
        payload.get("requirements") == GATE_REQUIREMENTS,
        "V13 receipt requirements differ",
    )
    inputs = (
        validate_v13_train_inputs(ssd_root=ssd_root)
        if input_contract is None
        else dict(input_contract)
    )
    _require(
        launch_receipt is not None and completion_receipt is not None,
        "V13 receipt replay requires exact launch and completion receipts",
    )
    checkpoint = validate_v13_checkpoint(
        memory_dir,
    ...
    _require(payload.get("checkpoint") == checkpoint, "V13 receipt checkpoint differs")
    _require(
        payload.get("training_provenance") == checkpoint["training_provenance"],
        "V13 receipt training provenance differs",
    )
    _require(
        payload.get("training_sources") == inputs["artifacts"],
        "V13 receipt sources differ",
    )
    _require(
        payload.get("v10_diagnostic_baseline")
        == inputs["v10_diagnostic_baseline"],
        "V13 receipt V10 diagnostic differs",
    )
    ...
    _require(
        ...
        "V13 receipt evaluation fingerprint differs from live inputs",
    )
    outputs = payload.get("outputs")
    _require(isinstance(outputs, Mapping), "V13 receipt outputs missing")
    manifest_path = _verify_artifact_binding(
        outputs.get("manifest", {}),
        description="V13 receipt manifest",
        ssd_root=ssd_root,
    )
    summary_path = _verify_artifact_binding(
        outputs.get("summary", {}),
        description="V13 receipt summary",
        ssd_root=ssd_root,
    )
    manifest = validate_existing_manifest(
        _load_json(manifest_path, description="V13 receipt manifest"),
        expected_fingerprint=fingerprint,
        expected_fingerprint_payload=fingerprint_payload,
        require_postload=True,
    )
    summary = _load_json(summary_path, description="V13 receipt summary")
    _require(summary.get("schema") == GATE_SUMMARY_SCHEMA, "V13 summary schema differs")
    _validate_self_hash(summary, field="summary_sha256")
    _require(
        manifest.get("fingerprint") == fingerprint
        and summary.get("fingerprint") == fingerprint,
        "V13 output fingerprints differ",
    )
    bindings = outputs.get("conditions")
    _require(
        isinstance(bindings, Mapping) and set(bindings) == set(CONDITIONS),
        "V13 condition bindings differ",
    )
    ...
    for condition in CONDITIONS:
        ...
        indexed = validate_resume_records(
            _read_jsonl(path, description=f"V13 {condition}"),
            condition=condition,
        ...
        _require(
            ...
            f"V13 {condition} output must contain exactly Value14",
        )
        records[condition] = [indexed[index] for index in VALUE14_ORDINALS]
    recomputed = build_v13_gate(
    ...
    _require(
        ...
        "V13 gate does not reproduce",
    )
    _require(payload.get("code") == evaluator_code_binding(), "V13 evaluator code differs")
    ...
    _require(
        ...
        "V13 candidate designation differs",
    )
    _require(
        ...
        "V13 receipt training authorization differs",
    )
    result = dict(payload)
...
def validate_continuation_authorization(*args: Any, **kwargs: Any) -> dict[str, Any]:
    raise V13EvaluationContractError(
        "V13 is a terminal fresh four-cycle run; no gate authorizes continuation"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _require(args.max_new_tokens == GATE_MAX_NEW_TOKENS, "V13 gate requires 128 tokens")
    _require(
        args.expected_memory_layer_count == GATE_EXPECTED_MEMORY_LAYER_COUNT,
        "V13 gate requires all 42 layers",
    )
    _require(args.device == GATE_DEVICE, "V13 gate device differs")
    _require(args.dtype == GATE_DTYPE, "V13 gate dtype differs")
    _require(
        args.attn_implementation == GATE_ATTN_IMPLEMENTATION,
        "V13 gate attention implementation differs",
    )
    _require(
        args.normal_fusion_profile == GATE_NORMAL_FUSION_PROFILE,
        "V13 gate fusion profile differs",
    )
    args.delta_mem_root = str(Path(args.delta_mem_root).expanduser().resolve())
    _require(Path(args.delta_mem_root) == PROJECT_ROOT, "V13 gate requires this checkout")
    ...
    inputs = validate_v13_train_inputs()
    ...
    _require(expected_layers == 42, "V13 resolved memory layer count differs")
    fingerprint_payload = build_evaluation_fingerprint_payload(
        input_contract=inputs,
        checkpoint=checkpoint,
    ...
    if paths["manifest"].exists():
        manifest = validate_existing_manifest(
            _load_json(paths["manifest"], description="existing V13 manifest"),
            expected_fingerprint=fingerprint,
            expected_fingerprint_payload=fingerprint_payload,
        )
    else:
    ...
    for condition in CONDITIONS:
        existing = (
            _read_jsonl(paths[condition], description=f"V13 {condition}")
