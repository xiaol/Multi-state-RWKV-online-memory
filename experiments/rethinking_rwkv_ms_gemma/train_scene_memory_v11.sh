...
RUN_ROOT="${SSD_ROOT}/delta_mem_outputs/novel_rwkv_ms_memory/scene_memory_v11"
GATES_ROOT="${RUN_ROOT}/gates"
CACHE_ROOT="${SSD_ROOT}/delta_mem_cache/scene_memory_v11"
...
[[ "${TARGET_STEP}" == "1" ]] || fail "v11_target_step_must_be_one"
[[ -z "${RESUME_FROM_CHECKPOINT}" ]] || fail "v11_resume_is_forbidden"
[[ -z "${GATE_RECEIPT}" ]] || fail "v11_gate_receipt_cannot_authorize_training"
...
RUN_ID="scene_memory_v11_${run_kind}_${RUN_NAME}_step1"
...
if [[ "${DRY_RUN}" == "0" ]]; then
  [[ -z "$(git -C "${REPO}" status --porcelain --untracked-files=no)" ]] \
    || fail "tracked_worktree_must_be_clean_before_v11_training"
fi
...
if [[ "${DRY_RUN}" == "0" ]]; then
  for critical_file in "${critical_tracked_files[@]}"; do
    git -C "${REPO}" ls-files --error-unmatch -- "${critical_file}" >/dev/null 2>&1 \
      || fail "critical_v11_source_must_be_tracked path=${critical_file}"
  done
fi
...
contract_args=(
  -m experiments.rethinking_rwkv_ms_gemma.scene_memory_v11_launch_contract
  --target-step 1
  --data-root "${DATA_ROOT}"
...
if ! contract_values="$("${VALIDATION_PYTHON_BIN}" "${contract_args[@]}")"; then
  fail "v11_launch_contract_validation_failed"
fi
...
[[ "${VALIDATED_TARGET_STEP}" == "1" ]] || fail "validated_target_step_differs"
[[ "${SOURCE_STEP}" == "0" && "${RESUME_SCHEDULE_CURSOR}" == "0" ]] \
  || fail "v11_must_begin_fresh_at_zero"
...
train_args=(
  ...
  --warm-start-mode scene_memory_v11_v8_checkpoint56_adapter_only
  --resume-mode exact
  ...
  --scene-state-generation-objective-version scene_state_generation_ce_symmetric_cycle_suffix_repair_v5
...
printf 'Validated V11 fresh one-cycle launch; first pair=%s,%s\n' \
...
printf 'Hard32, other benchmarks, resume, and second-cycle training are forbidden.\n'

if [[ "${DRY_RUN}" == "1" ]]; then
  printf 'Validated V11 training command (not started):\n'
...
checkpoint = trainer_output_path / "trainer/checkpoint-1"
...
payload = {
    ...
    "max_grad_norm": 1.0,
    "first_cycle_pairs": [list(pair) for pair in launch.FIRST_CYCLE_PAIRS],
    "first_cycle_pairs_sha256": launch.FIRST_CYCLE_PAIRS_SHA256,
...

{
  ...
  printf 'Starting attached V11 one-cycle training. Command: '
  printf '%q ' "${train_command[@]}"
  printf '\n'
} | tee "${LOG_FILE}"
...
set -e
(( pipeline_status[0] == 0 )) \
  || fail "v11_training_failed exit_code=${pipeline_status[0]} log=${LOG_FILE}"
(( pipeline_status[1] == 0 )) \
  || fail "v11_training_log_failed exit_code=${pipeline_status[1]}"
[[ -s "${OUTPUT_DIR}/training_summary.json" ]] \
  || fail "completed_training_summary_missing"
CHECKPOINT_DIR="${OUTPUT_DIR}/trainer/checkpoint-1"
...
payload = {
    ...
    "status": "completed",
    "optimizer_step": 1,
    "consumed_pair_presentations": checkpoint_contract["consumed_pair_presentations"],
    "launch_receipt": launch_validation["artifact"],
    "launch_receipt_sha256": launch_validation["receipt_sha256"],
    "log": launch.artifact_binding(Path(log_raw), description="v11_completion_log"),
...
printf 'Completed attached V11 optimizer checkpoint-1: %s\n' "${OUTPUT_DIR}"
printf 'Gate provenance: --launch-receipt %s --completion-receipt %s\n' \
  "${LAUNCH_RECEIPT}" "${COMPLETION_RECEIPT}"
printf 'Run only the V11 Train32/Value14 candidate gate next; never resume this run.\n'
