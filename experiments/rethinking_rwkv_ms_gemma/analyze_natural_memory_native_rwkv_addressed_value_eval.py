#!/usr/bin/env python3
"""Aggregate and sign the addressed-value native causal evaluation."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_rwkv_addressed_value_eval as evaluation,
)


SCHEMA = "rwkv_ms_natural_memory_native_addressed_value_result.v1"
...
def read_records(root: Path) -> tuple[dict[str, dict[int, Mapping[str, Any]]], list[Mapping[str, Any]]]:
    ...
    if len(partition_dirs) != evaluation.WORLD_SIZE * 2:
        raise ValueError(
            f"Expected eight addressed-value partitions, found {len(partition_dirs)}"
        )
    for partition_dir in partition_dirs:
        binding_path = partition_dir / "input_binding.json"
        if not binding_path.is_file():
            raise ValueError(f"Missing addressed-value binding: {binding_path}")
        ...
        if (
            ...
            or binding.get("partitions_per_shard") != 2
            or binding.get("conditions") != list(evaluation.CONDITIONS)
        ):
            raise ValueError(f"Addressed-value binding differs: {binding_path}")
        ...
        for condition in evaluation.CONDITIONS:
            ...
            if not path.is_file():
                raise ValueError(f"Missing addressed-value predictions: {path}")
            rows = 0
            for raw_line in path.read_text(encoding="utf-8").splitlines():
                ...
                if any(record.get(key) != value for key, value in required.items()):
                    raise ValueError(
                        f"Addressed-value prediction binding differs: {path}:{source_index}"
                    )
                if source_index in by_condition[condition]:
                    raise ValueError(
                        f"Duplicate addressed-value prediction: {condition}:{source_index}"
                    )
    ...
    if len(expected_indices) != evaluation.EVALUATION_ROWS:
        raise ValueError("Addressed-value correct-state row count differs")
    for condition, records in by_condition.items():
        if set(records) != expected_indices:
            raise ValueError(f"Addressed-value condition row set differs: {condition}")
    return by_condition, artifact_manifest


def aggregate_condition(records: Mapping[int, Mapping[str, Any]]) -> Mapping[str, Any]:
    tp = sum(int(record["score"]["tp"]) for record in records.values())
    fp = sum(int(record["score"]["fp"]) for record in records.values())
    fn = sum(int(record["score"]["fn"]) for record in records.values())
    covered = sum(bool(record["score"]["covered"]) for record in records.values())
...
def analyze(root: Path) -> Mapping[str, Any]:
    ...
    status = (
        ...
        if native_gain
        else "addressed_value_native_gain_not_established"
    )
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "status": status,
...
def write_result(path: Path, result: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Addressed-value result output must be fresh: {path}")
    path.write_text(
