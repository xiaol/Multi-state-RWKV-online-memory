#!/usr/bin/env python3
"""Analyze and sign the locked on-policy repair candidate."""
...
from experiments.rethinking_rwkv_ms_gemma import (  # noqa: E402
    run_natural_memory_native_scene_onpolicy_repair_eval as runner,
)


SCHEMA = "rwkv_ms_natural_memory_native_scene_onpolicy_repair_result.v1"
...
def analyze(**kwargs: Any) -> Mapping[str, Any]:
    ...
    result["decision"] = (
        "TRAIN-only on-policy repair endpoint cleared every preregistered gate; external replication still requires a separate protocol."
        if result["status"] == "passed"
        else "TRAIN-only on-policy repair endpoint failed at least one preregistered gate; archive without external replication."
    )
    result["receipt"] = {
        "algorithm": "sha256",
