#!/usr/bin/env python3
"""Generate correct-state outputs for locked native scene checkpoint soups."""
...
SCHEMA = "rwkv_ms_natural_memory_native_scene_checkpoint_soup_shard.v1"
INPUT_SCHEMA = "rwkv_ms_natural_memory_native_scene_checkpoint_soup_input.v1"
...
def validate_materialization(root: Path) -> tuple[Mapping[str, Any], list[Mapping[str, Any]]]:
    result = probe.validate_signed_json(
        root / "result.json",
        description="Checkpoint-soup materialization result",
    )
    ...
    if (
        ...
        raise ValueError("Checkpoint-soup materialization binding differs")
    candidates = result.get("candidates")
    if not isinstance(candidates, list) or len(candidates) != len(expected_ids):
        raise ValueError("Checkpoint-soup materialization candidates differ")
    ...
    for candidate_id, result_manifest in zip(expected_ids, candidates, strict=True):
        ...
        manifest = probe.validate_signed_json(
            ...
            description=f"Checkpoint-soup manifest {candidate_id}",
        )
        ...
        if (
            ...
            raise ValueError(f"Checkpoint-soup manifest binding differs: {candidate_id}")
...
def load_candidate_patch(
    ...
    *,
    ...
    if not isinstance(payload, Mapping) or not isinstance(payload.get("state_dict"), Mapping):
        raise ValueError("Checkpoint-soup patch payload differs")
    ...
    if any(payload.get(key) != value for key, value in required.items()):
        raise ValueError("Checkpoint-soup patch metadata differs")
    if runtime._state_dict_sha256(state) != manifest["gate_state_sha256"]:
        raise ValueError("Checkpoint-soup patch state hash differs")
    ...
    if set(state) != names or len(names) != 126:
        raise ValueError("Checkpoint-soup runtime gate names differ")
    with torch.no_grad():
        for name in sorted(names):
            source = state[name]
            target = parameters[name]
            if source.shape != target.shape:
                raise ValueError(f"Checkpoint-soup runtime shape differs: {name}")
    ...
    if runtime_sha256 != runtime._state_dict_sha256(expected):
        raise ValueError("Checkpoint-soup runtime-cast state differs")
    return {
        "candidate_id": manifest["candidate_id"],
        "gate_state_sha256": manifest["gate_state_sha256"],
...
def input_binding(
    *,
    ...
    if sha256_file(base_model / "config.json") != training.BASE_CONFIG_SHA256:
        raise ValueError("Checkpoint-soup base config differs")
...
def write_or_validate_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.is_file():
        if json.loads(path.read_text(encoding="utf-8")) != value:
            raise ValueError(f"Checkpoint-soup input binding differs: {path}")
        return
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        if raw_line.strip():
            ...
            if source_index in records:
                raise ValueError(f"Duplicate checkpoint-soup output: {path}:{source_index}")
...
def validate_resume(
    ...
    *,
    ...
    if not set(existing) <= set(expected):
        raise ValueError(f"Unexpected checkpoint-soup rows: {candidate_id}")
    for source_index, record in existing.items():
        ...
        if any(record.get(key) != value for key, value in required.items()):
            raise ValueError(f"Resumed checkpoint-soup output differs: {candidate_id}:{source_index}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not 0 <= args.shard_index < WORLD_SIZE:
        raise ValueError("Checkpoint-soup evaluation requires a valid four-way shard")
    ...
    if all(len(records) == len(shard_rows) for records in completed.values()):
        print(f"SCENE_CHECKPOINT_SOUP_SHARD_COMPLETE shard={args.shard_index}", flush=True)
        return 0
    ...
    for manifest in manifests:
        ...
        for ordinal, row in enumerate(shard_rows, start=1):
            ...
            print(
                f"SCENE_CHECKPOINT_SOUP_PROGRESS shard={args.shard_index} "
