#!/usr/bin/env python3
"""Aggregate and sign the scalar-agreement native generation benchmark."""
...
SCHEMA = "rwkv_ms_natural_memory_native_scalar_agreement_result.v1"
...
def read_records(
    ...
    if len(partition_dirs) != evaluation.WORLD_SIZE * PARTITIONS_PER_SHARD:
        raise ValueError(
            f"Expected four scalar-agreement partitions, found {len(partition_dirs)}"
        )
    for partition_dir in partition_dirs:
        binding_path = partition_dir / "input_binding.json"
        if not binding_path.is_file():
            raise ValueError(f"Missing scalar-agreement binding: {binding_path}")
        ...
        if (
            ...
            or binding.get("hybrid_mode") != "scalar_gate"
            ...
            or binding.get("rwkv_recurrent_scalar_controller_active") is not True
            or binding.get(
                "zero_recurrent_identity_requires_explicit_bypass_match"
            )
            is not True
        ):
            raise ValueError(f"Scalar-agreement binding differs: {binding_path}")
        ...
        for condition in evaluation.CONDITIONS:
            ...
            if not path.is_file():
                raise ValueError(f"Missing scalar-agreement predictions: {path}")
            rows = 0
            for raw_line in path.read_text(encoding="utf-8").splitlines():
                ...
                if any(record.get(key) != value for key, value in required.items()):
                    raise ValueError(
                        f"Scalar-agreement prediction binding differs: "
                        f"{path}:{source_index}"
                    )
                if source_index in by_condition[condition]:
                    raise ValueError(
                        f"Duplicate scalar-agreement prediction: "
    ...
    if len(expected_indices) != evaluation.EVALUATION_ROWS:
        raise ValueError("Scalar-agreement correct-state row count differs")
    for condition, records in by_condition.items():
        if set(records) != expected_indices:
            raise ValueError(f"Scalar-agreement condition row set differs: {condition}")
...
def analyze(root: Path) -> Mapping[str, Any]:
    ...
    status = (
        "scalar_agreement_native_recurrent_causal_gain_established"
        if passed
        else "scalar_agreement_native_gain_without_full_causal_pass"
        if native_gain
        else "scalar_agreement_native_gain_not_established"
    )
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "status": status,
...
def write_result(path: Path, result: Mapping[str, Any]) -> None:
    if path.exists():
        raise ValueError(f"Scalar-agreement result output must be fresh: {path}")
    path.write_text(
