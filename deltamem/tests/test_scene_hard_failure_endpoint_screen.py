...
def _selector_receipt(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "selected_checkpoint": {
            "global_step": evaluated_steps[-1],
            "path": f"/unit/checkpoint-{evaluated_steps[-1]}",
        },
        "authorization": {
...
def test_screen_stops_generation_at_first_passing_endpoint(tmp_path: Path) -> None:
    ...
    def select(
        ...
        return _selector_receipt(evaluated_steps=[16, 32], passed=True)
...
def test_screen_finishes_four_failures_and_writes_unauthorized_receipt(
    ...
    run_root, completion = _production_run(tmp_path)
    reports = {
        step: _gate_report(passed=False, step=step)
