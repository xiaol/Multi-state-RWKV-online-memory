...
from experiments.rethinking_rwkv_ms_gemma import (
    run_natural_memory_native_rwkv_alignment_residual_causal_train as alignment,
)
from experiments.rethinking_rwkv_ms_gemma import (
...
def test_protocol_binds_alignment_screen_and_fresh_endpoint() -> None:
    protocol = alignment.validate_protocol()
    screen_result = alignment.validate_screen_result()

    assert protocol["receipt"]["payload_sha256"] == (
        alignment.PROTOCOL_PAYLOAD_SHA256
    )
    assert protocol["authorization_basis"]["screen_result_file_sha256"] == (
        alignment.SCREEN_RESULT_FILE_SHA256
    )
    assert protocol["authorization_basis"]["screen_result_receipt"] == (
        alignment.SCREEN_RESULT_RECEIPT
    )
    assert screen_result["receipt"]["payload_sha256"] == (
        alignment.SCREEN_RESULT_RECEIPT
    )
    assert protocol["heldout_causal_endpoint"]["source_ordinals"] == list(
        alignment.HELDOUT_ORDINALS
    )
    assert protocol["heldout_causal_endpoint"]["source_donor_payload_sha256"] == (
        alignment.HELDOUT_PAYLOAD_SHA256
    )
...
def test_training_bindings_replace_all_run_inputs_then_restore() -> None:
    ...
    with alignment.training_bindings():
        assert shared.MODEL_LOADER is alignment.load_model
        assert shared.SELECTED_CANDIDATE == alignment.SELECTED_CANDIDATE
        assert shared.TRAINING_FUNCTION is causal.train
        assert shared.CALIBRATION_RESULT == alignment.SCREEN_RESULT
        assert (
            shared.CALIBRATION_RESULT_FILE_SHA256
            == alignment.SCREEN_RESULT_FILE_SHA256
        )
        assert shared.CALIBRATION_RESULT_RECEIPT == alignment.SCREEN_RESULT_RECEIPT
        assert shared.validate_calibration_result is alignment.validate_screen_result
